...
describe('computeDRE — despesas operacionais', () => {
  ...
  it('despesasOperacionais = 50 (d1 + d2)', () =>
    expect(result.despesasOperacionais).toBe(50));
...
describe('computeDRE — múltiplos pedidos', () => {
  const pedidos = [
    makePedido({ data: '2026-06-01', receita: 50,  custoTotal: 10, taxaShopee: 5,  dasImposto: 3, adsMarketing: 1, unidadesEstoque: 1 }),
    makePedido({ data: '2026-06-15', receita: 100, custoTotal: 20, taxaShopee: 10, dasImposto: 6, adsMarketing: 2, unidadesEstoque: 2 }),
    makePedido({ data: '2026-06-30', receita: 150, custoTotal: 30, taxaShopee: 15, dasImposto: 9, adsMarketing: 3, unidadesEstoque: 3 }),
  ];
  ...
  it('ticketMedio = 100',         () => expect(result.ticketMedio).toBe(100));
  it('margemPercentual = 62%',    () =>
    expect(result.margemPercentual).toBeCloseTo(62, 5));
});
