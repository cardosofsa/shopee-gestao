...
import {
  ArrowLeft, Package, TrendingUp, ShoppingCart, Zap,
  AlertTriangle, ChevronDown, ChevronUp, Calculator, Download,
} from 'lucide-react';
import {
...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-xs">
      ...
      {payload.map((p: any) => (
        <p key={p.name} style={{ color: p.color }}>{p.name}: {fmt(Number(p.value))}</p>
      ))}
    </div>
  );
}
...
function KCard({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: string }) {
  return (
...
export default function ProdutoDetalhe() {
  ...
  const vendas = useMemo(
    () => pedidosAll.filter((p) => p.sku === sku && (p.status === 'Concluído' || p.status === 'Enviado')),
    [pedidosAll, sku],
  );
  ...
  const vdDia7  = useMemo(() => vendas.filter((p) => p.data >= limite7d).reduce((s, p) => s + p.unidadesEstoque, 0) / 7,  [vendas, limite7d]);
  const vdDia14 = useMemo(() => vendas.filter((p) => p.data >= limite14d).reduce((s, p) => s + p.unidadesEstoque, 0) / 14, [vendas, limite14d]);
  const vdDia30 = useMemo(() => vendas.filter((p) => p.data >= limite30d).reduce((s, p) => s + p.unidadesEstoque, 0) / 30, [vendas, limite30d]);
  ...
  const pontoReposicao = useMemo(() => vdDia > 0
    ? Math.ceil(vdDia * (LEAD_TIME_PADRAO + (produto?.estoqueSeguranca ?? 0)))
    : null,
  [vdDia, produto]);
  ...
  const status = useMemo(
    () => produto ? getStatusEstoque(produto.estoqueAtual, vdDia, produto.estoqueSeguranca) : 'Estoque Estável' as StatusEstoque,
    [produto, vdDia],
  );
  ...
  const vendasMes = useMemo(() => {
    ...
    vendas.forEach((p) => {
      ...
      map.set(mes, { fat: prev.fat + p.receita, lucro: prev.lucro + p.lucroOperacional, qtd: prev.qtd + p.unidadesEstoque });
    });
    return Array.from(map.entries())
      .sort((a, b) => a[0].localeCompare(b[0]))
      .map(([mes, d]) => ({ name: fmtMes(mes), Faturamento: d.fat, 'Lucro Op.': d.lucro, Unidades: d.qtd }));
  }, [vendas]);
  ...
  const topClientes = useMemo(() => {
    ...
    todasVendas
      ...
      .forEach((p) => {
        ...
        map.set(key, { pedidos: prev.pedidos + 1, receita: prev.receita + p.receita, lucro: prev.lucro + p.lucroOperacional });
        (map as any)._names ??= {};
        (map as any)._names[key] = nome;
      });
    return [...map.entries()]
      .map(([key, v]) => ({ nome: (map as any)._names?.[key] ?? key, ...v }))
    ...
    exportXlsx(`produto_${sku}_${new Date().toISOString().slice(0, 10)}.xlsx`, [
      {
        ...
        rows: todasVendas.map((p) => [p.data, p.numeroPedido, p.nomeCliente ?? '', p.unidadesEstoque, xlsxNum(p.receita), xlsxNum(p.lucroOperacional), p.status]),
      },
      {
        name: 'Compras',
        headers: ['Data', 'Qtd', 'Custo Unit.', 'Total', 'Fornecedor', 'Pagamento'],
        rows: comprasSku.map((c) => [c.data, c.quantidadeEntrada, xlsxNum(c.custoUnitario), xlsxNum(c.custoTotal), c.fornecedor, c.pagamento]),
      },
    ]);
  };
  ...
  if (!produto) {
    return (
      <div className="p-6">
        <Link to="/estoque" className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 mb-6 transition-colors">
          <ArrowLeft size={14} /> Voltar ao Estoque
        </Link>
        <div className="card p-12 text-center">
          <Package size={32} className="text-slate-300 mx-auto mb-3" />
          <p className="text-slate-500 dark:text-slate-400">Produto não encontrado: <span className="font-mono font-bold">{sku}</span></p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2 text-sm">
          <Link to="/estoque" className="inline-flex items-center gap-1.5 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors">
            <ArrowLeft size={14} /> Estoque
          </Link>
          <span className="text-slate-300 dark:text-slate-600">/</span>
          <span className="font-mono font-medium text-slate-700 dark:text-slate-300">{sku}</span>
        </div>
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              <span className="font-mono text-xs text-slate-400 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded">{produto.sku}</span>
              {produto.categoria && (
                <span className="text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-700/50 px-2 py-0.5 rounded">{produto.categoria}</span>
              )}
              <span className="text-xs text-slate-400 dark:text-slate-500">{produto.loja}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${STATUS_STYLE[status]}`}>{status}</span>
            <span className={`inline-flex items-center justify-center w-7 h-7 rounded-lg text-xs font-bold ${ABC_STYLE[String(curvaABC)] ?? ABC_STYLE['—']}`} title="Curva ABC">{curvaABC}</span>
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${produto.ativo ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <KCard label="Estoque Atual" value={String(produto.estoqueAtual)} sub={`Seg.: ${produto.estoqueSeguranca} un.`} />
        <KCard
          label="Cobertura"
          value={isFinite(diasCobertura) ? `${Math.round(diasCobertura)}d` : '∞'}
          sub="dias de cobertura"
          accent={
            !isFinite(diasCobertura) ? 'text-slate-300 dark:text-slate-600' :
            diasCobertura < 7  ? 'text-red-500' :
            diasCobertura < 30 ? 'text-amber-500' : 'text-emerald-600'
          }
        />
        <KCard label="Valor em Estoque" value={fmt(valorEstoque)} sub={`Custo: ${fmt(produto.custoUnitario)}/un.`} />
        <KCard label="Venda / Dia (30d)" value={vdDia > 0 ? vdDia.toFixed(2) : '—'} sub="últ. 30 dias" />
      </div>

      {/* KPI Row 2 — Financeiro */}
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <KCard label="Faturamento Total" value={fmt(totais.faturamento)} sub={`${totais.pedidos} pedido${totais.pedidos !== 1 ? 's' : ''}`} />
        <KCard
        ...
        <KCard
          ...
          sub="sobre receita"
          accent={totais.margem >= 20 ? 'text-emerald-600' : totais.margem >= 10 ? 'text-amber-500' : 'text-red-500'}
        />
        <KCard
          label="ROI"
          value={totais.roi > 0 ? fmtPct(totais.roi) : '—'}
          sub="retorno sobre custo"
          accent={totais.roi >= 30 ? 'text-emerald-600' : totais.roi >= 15 ? 'text-amber-500' : 'text-red-500'}
        />
      </div>
      ...
      <div className="card p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Inteligência de Reposição</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">

          {/* Velocidade */}
          <div className="bg-slate-50 dark:bg-slate-700/40 rounded-xl p-4">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-2.5">Velocidade de Venda</p>
            <div className="space-y-2">
              {[{ label: '7 dias', v: vdDia7 }, { label: '14 dias', v: vdDia14 }, { label: '30 dias', v: vdDia30 }].map(({ label, v }) => (
                <div key={label} className="flex items-center gap-2">
                  <span className="text-xs text-slate-400 w-14 flex-shrink-0">{label}</span>
                  <div className="flex-1 h-1.5 bg-slate-200 dark:bg-slate-600 rounded-full overflow-hidden">
                    <div className="h-full bg-core-green rounded-full transition-all" style={{ width: `${Math.min(100, (v / Math.max(vdDia7, vdDia14, vdDia30, 0.01)) * 100)}%` }} />
                  </div>
                  <span className="text-xs font-bold text-slate-700 dark:text-slate-200 w-12 text-right">{v > 0 ? `${v.toFixed(2)}/d` : '—'}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Ponto de Reposição */}
          <div className="bg-slate-50 dark:bg-slate-700/40 rounded-xl p-4">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-2.5">Ponto de Reposição</p>
            {pontoReposicao !== null ? (
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-baseline">
                  ...
                  <span className="text-xs text-slate-600 dark:text-slate-300">{LEAD_TIME_PADRAO} dias</span>
                </div>
                <div className="flex justify-between items-baseline">
                  <span className="text-xs text-slate-500 dark:text-slate-400">Cobertura alvo</span>
                  <span className="text-xs text-slate-600 dark:text-slate-300">{COBERTURA_ALVO} dias</span>
                </div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500">Sem dados suficientes de venda para calcular.</p>
            )}
          </div>

          {/* Custo investido compras */}
          <div className="bg-slate-50 dark:bg-slate-700/40 rounded-xl p-4">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-2.5">Resumo de Compras</p>
            <div className="space-y-2">
              <div className="flex justify-between items-baseline">
              ...
              {comprasSku[0] && (
                <div className="flex justify-between items-baseline">
                  ...
                  <span className="text-xs text-slate-600 dark:text-slate-300">{comprasSku[0].data}</span>
                </div>
              )}
              {comprasSku.length > 1 && (
                <div className="flex justify-between items-baseline">
                  ...
                  <span className="text-xs font-medium text-slate-600 dark:text-slate-300">
                    {fmt(comprasSku.reduce((s, c) => s + c.custoUnitario, 0) / comprasSku.length)}/un.
                  </span>
                </div>
              )}
              <div className="flex justify-between items-baseline">
                <span className="text-xs text-slate-500 dark:text-slate-400">Nº compras</span>
                <span className="text-xs text-slate-600 dark:text-slate-300">{comprasSku.length}</span>
              </div>
            </div>
          </div>
      ...
      <div className="card overflow-hidden">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">Simulador de Preço</span>
            <span className="text-xs text-slate-400 dark:text-slate-500">· custo atual: {fmt(produto.custoUnitario)}</span>
          </div>
          {showSim ? <ChevronUp size={14} className="text-slate-400" /> : <ChevronDown size={14} className="text-slate-400" />}
        </button>
        {showSim && simPrecos && (
          <div className="px-5 pb-5 space-y-4 border-t border-slate-100 dark:border-slate-700 pt-4">
            <div className="flex items-center gap-3 flex-wrap">
              <label className="text-sm text-slate-600 dark:text-slate-300 font-medium">Margem alvo:</label>
              <div className="flex items-center gap-2">
                <input
                  type="range" min={0} max={50} step={1} value={simMargem}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                { label: 'Break-even', v: simPrecos.breakEven, sub: '0% de margem', cls: 'text-slate-600 dark:text-slate-300' },
                { label: `Margem ${simMargem}% (alvo)`, v: simPrecos.alvo, sub: 'preço sugerido', cls: 'text-core-green' },
                { label: 'Margem 30%', v: simPrecos.ultra, sub: 'premium', cls: 'text-emerald-600' },
              ].map(({ label, v, sub, cls }) => (
            ...
            <p className="text-[10px] text-slate-400 dark:text-slate-500">
              Cálculo: comissão marketplace 20% + DAS {configuracoes.aliquotaDAS ?? 0}% + Ads {configuracoes.percentualMarketing ?? 2}% + custo {fmt(simPrecos.custo)}
            </p>
          </div>
        )}
      </div>
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={140}>
            <LineChart data={evolucaoCusto} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={C.grid} vertical={false} />
              <XAxis dataKey="name" tick={{ fontSize: 11, fill: C.slate }} axisLine={false} tickLine={false} />
              <YAxis tickFormatter={(v) => `R$${v.toFixed(2)}`} tick={{ fontSize: 11, fill: C.slate }} axisLine={false} tickLine={false} width={56} />
              <Tooltip formatter={(v: any) => [fmt(Number(v)), 'Custo Unit.']} contentStyle={{ fontSize: 12 }} />
              <Line type="monotone" dataKey="Custo Unit." stroke={C.orange} strokeWidth={2} dot={{ r: 4, fill: C.orange }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
          ...
          <div className="space-y-2">
            {topClientes.map((c, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <span className="flex-1 text-sm text-slate-700 dark:text-slate-200 truncate">{c.nome}</span>
                <span className="text-xs text-slate-500 dark:text-slate-400">{c.pedidos}× ped.</span>
                <span className="text-xs font-medium text-core-green w-24 text-right">{fmt(c.receita)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {todasVendas.length > 0 && (
        <div className="card overflow-hidden">
          ...
          >
            ...
            {showVendas ? <ChevronUp size={13} className="text-slate-400" /> : <ChevronDown size={13} className="text-slate-400" />}
          </button>
          {showVendas && (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700">
                  <tr>
                    {['Data', 'Nº Pedido', 'Cliente', 'Qtd.', 'Receita', 'Lucro Op.', 'Status'].map((h) => (
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                  {todasVendas.slice(0, 30).map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">
                      <td className="px-4 py-2.5 text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap">{p.data}</td>
                      <td className="px-4 py-2.5 text-xs font-mono text-slate-600 dark:text-slate-300">{p.numeroPedido || '—'}</td>
                      <td className="px-4 py-2.5 text-xs text-slate-500 dark:text-slate-400 max-w-32 truncate">{p.nomeCliente || '—'}</td>
                      <td className="px-4 py-2.5 font-medium text-slate-800 dark:text-slate-100">{p.unidadesEstoque}</td>
                      ...
                      <td className="px-4 py-2.5">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                          ...
                          p.status === 'Devolvido' ? 'bg-red-50 text-red-700' :
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                {comprasSku.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">
                    <td className="px-4 py-2.5 text-xs text-slate-500 dark:text-slate-400">{c.data}</td>
                    <td className="px-4 py-2.5 font-bold text-slate-800 dark:text-slate-100">{c.quantidadeEntrada}</td>
                    <td className="px-4 py-2.5 text-slate-600 dark:text-slate-300">{fmt(c.custoUnitario)}</td>
                    <td className="px-4 py-2.5 font-medium text-slate-800 dark:text-slate-100">{fmt(c.custoTotal)}</td>
                    <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400">{c.fornecedor || '—'}</td>
                    <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400">{c.pagamento}{c.parcelas > 1 ? ` ${c.parcelas}×` : ''}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700">
                <tr>
                  <td className="px-4 py-2.5 text-xs font-semibold text-slate-500 uppercase tracking-wide" colSpan={3}>Total investido</td>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                {ajustesSku.map((a) => (
                  <tr key={a.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">
                    <td className="px-4 py-2.5 text-xs text-slate-500 dark:text-slate-400">{a.criadoEm.slice(0, 10)}</td>
                    <td className="px-4 py-2.5">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${a.tipo === 'entrada' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'}`}>
                        {a.tipo === 'entrada' ? '+ Entrada' : '− Saída'}
                      </span>
                    </td>
                    <td className="px-4 py-2.5 font-bold text-slate-800 dark:text-slate-100">{a.quantidade}</td>
                    <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400">{a.estoqueAntes}</td>
                    <td className="px-4 py-2.5 font-medium text-slate-700 dark:text-slate-200">{a.estoqueDepois}</td>
                    <td className="px-4 py-2.5 text-xs text-slate-400 dark:text-slate-500">{a.motivo || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {todasVendas.length === 0 && comprasSku.length === 0 && ajustesSku.length === 0 && (
        <div className="card p-10 text-center">
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm">Nenhuma venda, compra ou ajuste registrado para este SKU.</p>
        </div>
      )}
