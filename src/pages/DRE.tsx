...
import {
  ...
  Minus, FileBarChart2, ToggleLeft, ToggleRight, Download,
  CheckCircle2, AlertTriangle, Lightbulb, X, Lock,
  AlertCircle, Info,
} from 'lucide-react';
        ...
        {segmentos.map((s, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-sm flex-shrink-0" style={{ backgroundColor: s.cor }} />
              <span className="text-[11px] text-slate-500 dark:text-slate-400">
                {s.label} <span className="font-medium text-slate-700 dark:text-slate-300">{pct.toFixed(1)}%</span>
              </span>
            </div>
          );
        })}
...
function DRELinha({ linha, showComp }: { linha: LinhaConfig; showComp: boolean }) {
  ...
  const corValor =
    tipo === 'total'
      ? destacar
        ? valorExibido >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'
        : 'text-slate-800 dark:text-slate-100'
      : tipo === 'subtotal'
        ? valorExibido >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'
        : negativo
  ...
  const deltaPct = delta !== undefined && valorAnt !== 0 ? (delta / Math.abs(valorAnt!)) * 100 : undefined;
  const DeltaIcon = deltaPct === undefined ? null
    : deltaPct > 2 ? TrendingUp : deltaPct < -2 ? TrendingDown : Minus;
  const deltaColor = deltaPct === undefined ? ''
    : (negativo ? deltaPct > 0 : deltaPct > 0) ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400';
  ...
  return (
    ...
    `}>
      ...
      <p className={`flex-1 text-[13px] leading-snug ${
        isSubOrTotal ? 'font-bold text-slate-800 dark:text-slate-100' : 'text-slate-600 dark:text-slate-300'
      }`}>
        {isSubOrTotal && <span className="text-slate-400 dark:text-slate-500 mr-1.5 font-normal text-[11px]">=</span>}
        {label}
      </p>

      {/* % da receita bruta */}
      <span className={`w-14 text-right text-[11px] font-mono ${
        isSubOrTotal ? 'font-semibold text-slate-500 dark:text-slate-400' : 'text-slate-400 dark:text-slate-500'
      }`}>
...
function computarDRE(
  ...
  const receitaBruta   = concluido.reduce((s: number, p: any) => s + p.receita, 0);
  const descontos      = concluido.reduce((s: number, p: any) => s + (p.desconto ?? 0), 0);
  const devolucoes     = devolvido.reduce((s: number, p: any) => s + p.receita, 0);
  const receitaLiquida = receitaBruta - descontos - devolucoes;
  const cmv            = concluido.reduce((s: number, p: any) => s + p.custoTotal, 0);
  const taxas          = concluido.reduce((s: number, p: any) => s + p.taxaShopee, 0);
  const ads            = concluido.reduce((s: number, p: any) => s + p.adsMarketing, 0);
  const das            = concluido.reduce((s: number, p: any) => s + p.dasImposto, 0);
  ...
  return {
    receitaBruta, descontos, devolucoes, receitaLiquida,
    cmv, taxas, ads, das, lucroOpBruto,
    despesasPorCat, despesasTotal, resultado, margem, margemOp, pedidosQtd,
  };
}
...
export default function DRE() {
  ...
  const analise = useMemo(() => generateAnaliseGestorOp({
  ...
  const linhas: LinhaConfig[] = useMemo(() => {
    ...
    const rows: LinhaConfig[] = [
      ...
      { tipo: 'item',     label: 'Vendas de produtos',            valor: dre.receitaBruta, valorAnt: dreAnt.receitaBruta,  base: b },
      ...(dre.descontos > 0  ? [{ tipo: 'item' as TipoLinha, label: 'Descontos concedidos', valor: dre.descontos,  valorAnt: dreAnt.descontos,  base: b, negativo: true }] : []),
      ...(dre.devolucoes > 0 ? [{ tipo: 'item' as TipoLinha, label: 'Devoluções / reembolsos', valor: dre.devolucoes, valorAnt: dreAnt.devolucoes, base: b, negativo: true }] : []),
      { tipo: 'subtotal', label: 'Receita Líquida',               valor: dre.receitaLiquida, valorAnt: dreAnt.receitaLiquida, base: b },

      { tipo: 'section',  label: 'Custos de Venda (CMV)',          valor: 0, base: b },
      { tipo: 'item',     label: 'Custo das Mercadorias (CMV)',    valor: dre.cmv,   valorAnt: dreAnt.cmv,   base: b, negativo: true },
      { tipo: 'item',     label: 'Taxas do marketplace',          valor: dre.taxas, valorAnt: dreAnt.taxas, base: b, negativo: true },
      { tipo: 'item',     label: 'Marketing / Ads',               valor: dre.ads,   valorAnt: dreAnt.ads,   base: b, negativo: true },
      { tipo: 'item',     label: 'Impostos (DAS / Simples)',       valor: dre.das,   valorAnt: dreAnt.das,   base: b, negativo: true },
      { tipo: 'subtotal', label: 'Lucro Operacional Bruto',       valor: dre.lucroOpBruto, valorAnt: dreAnt.lucroOpBruto, base: b, destacar: true },
      ...
      ...(Object.keys(dre.despesasPorCat).length > 0 ? [
        ...
        { tipo: 'subtotal' as TipoLinha, label: 'Total Despesas Operacionais', valor: dre.despesasTotal, valorAnt: dreAnt.despesasTotal, base: b, negativo: true },
      ] : []),

      { tipo: 'total',    label: 'Resultado Líquido do Período',  valor: dre.resultado, valorAnt: dreAnt.resultado, base: b, destacar: true },
    ];
    return rows;
  }, [dre, dreAnt]);
    ...
    return (
      <div className="p-6">
        <div className="card p-14 text-center">
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm">Importe pedidos para gerar o DRE.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2.5 mb-0.5">
            ...
            <span className="text-xs text-slate-400 dark:text-slate-500">Demonstrativo de Resultados</span>
          </div>
              ...
              onClick={() => {
                const rows = linhas
                  ...
                  .map((l) => [
                    ...
                    dre.receitaBruta > 0 ? `${((Math.abs(l.valor) / dre.receitaBruta) * 100).toFixed(1)}%` : '',
                    xlsxNum(l.valor),
                  ]);
                exportXlsx(`DRE_${mes}`, [{
                  name: `DRE ${mes}`,
      ...
      {temDados && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Receita Bruta',        val: dre.receitaBruta,   ant: dreAnt.receitaBruta,   cor: 'border-t-slate-300' },
            { label: 'Lucro Op. Bruto',       val: dre.lucroOpBruto,   ant: dreAnt.lucroOpBruto,   cor: 'border-t-emerald-400' },
            { label: 'Margem Op.',            val: dre.margemOp,       ant: dreAnt.margemOp,       cor: 'border-t-blue-400',  isMargem: true },
            { label: 'Resultado Líquido',     val: dre.resultado,      ant: dreAnt.resultado,      cor: dre.resultado >= 0 ? 'border-t-core-green' : 'border-t-red-500' },
            ...
            return (
              <div key={label} className={`card border-t-[3px] ${cor} p-4`}>
                <p className={`text-lg font-bold leading-none ${
                  val < 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-800 dark:text-slate-100'
                }`}>
                  {isMargem ? `${val.toFixed(1)}%` : fmt(val)}
                </p>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1.5 font-medium">{label}</p>
                {showComp && ant !== 0 && (
                  <p className={`text-[10px] font-semibold mt-1 flex items-center gap-0.5 ${isPos ? 'text-emerald-600' : 'text-red-500'}`}>
      ...
      {/* ── DRE Inteligente ─────────────────────────────────────────────── */}
      {temDados && (analise.pontosFortes.length > 0 || analise.pontosAtencao.length > 0 || analise.recomendacoes.length > 0) && (
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(['Consultor E-commerce', 'Investidor'] as const).map(v => (
              ...
              >
                <Lock size={11} />
                {v}
                <span className="ml-0.5 text-[10px] font-bold bg-slate-100 dark:bg-slate-700 text-slate-400 px-1.5 py-0.5 rounded-full">Em breve</span>
              </button>
            ))}
          </div>
          ...
          {(analise.pontosFortes.length > 0 || analise.pontosAtencao.length > 0) && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="card p-4 space-y-2">
                ...
                {analise.pontosFortes.length === 0 ? (
                  <p className="text-xs text-slate-400 italic">Nenhum ponto forte identificado neste período.</p>
                ) : (
                  <ul className="space-y-1.5">
                    {analise.pontosFortes.map(pf => (
                      <li key={pf.id} className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-300">
                        <span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0" />
                        <span className="leading-snug">
                          {pf.texto}
                          {pf.valor && <span className="ml-1 font-bold text-emerald-600 dark:text-emerald-400">{pf.valor}</span>}
                        </span>
                      </li>
                    ))}
                  </ul>
              ...
              <div className="card p-4 space-y-2">
                ...
                {analise.pontosAtencao.length === 0 ? (
                  <p className="text-xs text-slate-400 italic">Nenhum ponto de atenção identificado.</p>
                ) : (
                  <ul className="space-y-1.5">
                    {analise.pontosAtencao.map(pa => (
                      <li key={pa.id} className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-300">
                        <span className="mt-0.5 w-1.5 h-1.5 rounded-full bg-amber-400 flex-shrink-0" />
                        <span className="leading-snug">
                          {pa.texto}
                          {pa.valor && <span className="ml-1 font-bold text-amber-600 dark:text-amber-400">{pa.valor}</span>}
                        </span>
                      </li>
                    ))}
                  </ul>
          ...
          {analise.recomendacoes.filter(r => !dismissed.has(r.id)).length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Recomendações</span>
              </div>
              {analise.recomendacoes
                .filter(r => !dismissed.has(r.id))
                .map((rec: Recomendacao) => {
                  const urgClr = rec.urgencia === 'alta'
                    ? { border: 'border-l-red-400', badge: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', icon: <AlertCircle size={12} /> }
                    : rec.urgencia === 'media'
                    ? { border: 'border-l-amber-400', badge: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400', icon: <AlertTriangle size={12} /> }
                    : { border: 'border-l-blue-300', badge: 'bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400', icon: <Info size={12} /> };
                    ...
                    <div key={rec.id} className={`card p-4 border-l-4 ${urgClr.border}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0 space-y-1.5">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wide ${urgClr.badge}`}>
                              {urgClr.icon}
                              {rec.urgencia}
                            </span>
                          </div>
                          <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 leading-snug">{rec.acao}</p>
                          <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">{rec.racional}</p>
                          {rec.impacto && (
                            <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">↑ {rec.impacto}</p>
                          )}
                        </div>
                        <button
          ...
          <p className="text-[11px] text-slate-400 dark:text-slate-500 flex items-start gap-1.5">
            ...
            Análise baseada nos dados disponíveis no sistema. Não substitui avaliação profissional contábil ou financeira.
          </p>
        </div>
      )}
      ...
      {temDados ? (
        <div className="card overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-5 py-3 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
            <p className="flex-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">Descrição</p>
            <p className="w-14 text-right text-[10px] font-bold uppercase tracking-wider text-slate-400">% RB</p>
            <p className="w-32 text-right text-[10px] font-bold uppercase tracking-wider text-slate-400">{mesLabelCurto(mes)}</p>
            {showComp && (
          ...
          <div className="px-5 py-3 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
            <p className="text-[10px] text-slate-400 dark:text-slate-500">
              % RB = percentual em relação à Receita Bruta · Pedidos com status Concluído e Enviado · Devolvidos excluídos do lucro
            </p>
          </div>
        </div>
      ) : (
