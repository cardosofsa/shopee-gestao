...
export function ComparativoContent({ embedded = false }: { embedded?: boolean }) {
  ...
  // Melhores / piores
  const melhorFat = useMemo(
    () =>
      meses.reduce(
        (best, m) =>
          (kpisPorMes[m]?.faturamento ?? 0) > (kpisPorMes[best]?.faturamento ?? 0) ? m : best,
        meses[0] ?? ''
      ),
    [meses, kpisPorMes]
  );
  const melhorLucro = useMemo(
    () =>
      meses.reduce(
        (best, m) => ((kpisPorMes[m]?.lucroOp ?? 0) > (kpisPorMes[best]?.lucroOp ?? 0) ? m : best),
        meses[0] ?? ''
      ),
    [meses, kpisPorMes]
  );
  const melhorMargem = useMemo(
    () =>
      meses.reduce(
        (best, m) => ((kpisPorMes[m]?.margem ?? 0) > (kpisPorMes[best]?.margem ?? 0) ? m : best),
        meses[0] ?? ''
      ),
    [meses, kpisPorMes]
  );
      ...
      <div className="p-6">
        <div className="card p-14 text-center">
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm">
            Importe pedidos para ver o comparativo mensal.
          </p>
        </div>
      </div>
    );
          ...
          {[
            {
              ...
              mes: melhorFat,
              val: fmt(kpisPorMes[melhorFat]?.faturamento ?? 0),
            },
            {
              label: 'Maior lucro op.',
              mes: melhorLucro,
              val: fmt(kpisPorMes[melhorLucro]?.lucroOp ?? 0),
            },
            {
              label: 'Maior margem op.',
              mes: melhorMargem,
              val: fmtPct(kpisPorMes[melhorMargem]?.margem ?? 0),
            },
          ].map(({ label, mes, val }) => (
            <div key={label} className="card p-4 border-t-[3px] border-t-core-green">
              ...
              <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-1 leading-none">
                {val}
              </p>
          ...
          <table className="w-full text-sm">
              ...
              <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700">
                ...
                {mesesVisiveis.map((m) => {
                  ...
                  return (
                    <th
                      key={m}
                      className={`px-4 py-3 text-right text-[11px] font-bold min-w-[130px] ${
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
              {METRICAS.map((metrica) => (
                ...
                >
                  <td
                    className={`px-4 py-2.5 sticky left-0 bg-white dark:bg-slate-800 ${
                      metrica.highlight
                  ...
                  {mesesVisiveis.map((m, i) => {
                    ...
                    const valAnt = antMes
                      ? ((kpisPorMes[antMes] as any)?.[metrica.key] ?? 0)
                      : null;
                    const dlt = valAnt !== null ? delta(val, valAnt) : null;
                    const isMelhor =
                      (metrica.key === 'faturamento' && m === melhorFat) ||
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col items-end gap-0.5">
                          ...
                          {dlt !== null && <DeltaChip pct={dlt} invert={metrica.invert} />}
                        </div>
                      </td>
                    );
                  })}
