...
import {
  Calculator, TrendingUp, Percent, Save, Download,
...
type CalcResult = {
  preco: number; taxaShopee: number; adsVal: number; dasVal: number;
  lucro: number; margemReal: number; totalCusto: number;
  com: number; fixo: number; tier: typeof SHOPEE_TIERS[0] | null;
};

function computeCalc(params: {
  custo: number; embalagem: number; frete: number; comissaoShopee: number;
  taxaFixa: number; aliquotaDAS: number; percentualAds: number;
  margemDesejada: number; modo: 'shopee' | 'avancado';
}): CalcResult | null {
  const { custo, embalagem, frete, comissaoShopee, taxaFixa, aliquotaDAS, percentualAds, margemDesejada, modo } = params;
  const ads = percentualAds / 100;
  const das = aliquotaDAS / 100;
  const mg  = margemDesejada / 100;
  const totalCusto = custo + embalagem + frete;

  let preco: number, com: number, fixo: number, tier: typeof SHOPEE_TIERS[0] | null = null;
...
function calcPrecoSim(preco: number, totalCusto: number, com: number, fixo: number, ads: number, das: number) {
...
function SliderField({
  label, value, onChange, min = 0, max = 100, step = 0.5, hint,
}: {
      ...
      <div className="flex items-center justify-between mb-1.5">
        ...
        <div className="flex items-center gap-1.5">
          <input type="number" min={min} max={max} step={step}
            className="w-16 px-2 py-1 border border-slate-200 rounded-md text-xs text-right text-slate-800 focus:outline-none focus:ring-1 focus:ring-shopee-400"
            value={value}
  ...
  return (
    <div className="flex flex-col items-center">
      <svg viewBox="0 0 100 60" className="w-36">
        <path d="M 12 52 A 38 38 0 0 1 88 52"
          fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="9" strokeLinecap="round"
          pathLength="100" />
        <path d="M 12 52 A 38 38 0 0 1 88 52"
          fill="none" stroke="white" strokeWidth="9" strokeLinecap="round"
          pathLength="100" strokeDasharray={`${clamped} 100`}
          style={{ opacity: 0.9 }} />
        <text x="50" y="46" textAnchor="middle" fontSize="16" fontWeight="bold" fill="white">
          {pct.toFixed(1)}%
        </text>
        <text x="10" y="58" textAnchor="middle" fontSize="6" fill="rgba(255,255,255,0.5)">0%</text>
        <text x="90" y="58" textAnchor="middle" fontSize="6" fill="rgba(255,255,255,0.5)">100%</text>
      </svg>
      <p className="text-[11px] text-shopee-100 -mt-0.5">Margem Líquida</p>
    </div>
  );
}
...
export default function Calculadora() {
  ...
  const [percentualAds, setPercentualAds] = useState(draft?.percentualAds ?? configuracoes.percentualMarketing);
  ...
  useEffect(() => {
    setDraft({ modo, modoReverso, skuRef, custo, embalagem, frete, percentualAds, aliquotaDAS, margemDesejada, comissaoShopee, taxaFixa, precoVenda });
  }, [modo, modoReverso, skuRef, custo, embalagem, frete, percentualAds, aliquotaDAS, margemDesejada, comissaoShopee, taxaFixa, precoVenda, setDraft]);
  ...
  const calc = useMemo(() => computeCalc({
    custo, embalagem, frete, comissaoShopee, taxaFixa,
    aliquotaDAS, percentualAds, margemDesejada, modo,
  }), [custo, embalagem, frete, comissaoShopee, taxaFixa, aliquotaDAS, percentualAds, margemDesejada, modo]);
  ...
  const calcReverso = useMemo(() => {
    ...
    if (modo === 'shopee') {
      com = 0.18;
      const tier = SHOPEE_TIERS.find((t) => precoVenda >= t.min && precoVenda <= t.max)
    ...
    return { lucro, margemReal: (lucro / precoVenda) * 100, taxaShopee, adsVal, dasVal, totalCusto };
  }, [modoReverso, precoVenda, custo, embalagem, frete, percentualAds, aliquotaDAS, comissaoShopee, taxaFixa, modo]);

  const simulacao = useMemo(() => {
    if (!calc) return [];
    const bases = [0.85, 0.90, 0.95, 1, 1.05, 1.10, 1.20].map((m) => calc.preco * m);
    return bases.map((p) => {
      const ads = percentualAds / 100;
      const das = aliquotaDAS / 100;
      let com: number, fixo: number;
      if (modo === 'shopee') {
        com = 0.18;
        const tier = SHOPEE_TIERS.find((t) => p >= t.min && p <= t.max)
          ?? SHOPEE_TIERS[SHOPEE_TIERS.length - 1];
        fixo = tier.fixed;
      } else {
        com = comissaoShopee / 100;
  ...
  const stackedItems = calc ? [
    { label: 'Custo Total',       value: calc.totalCusto, pct: (calc.totalCusto / calc.preco) * 100, barColor: 'bg-red-400',     textColor: 'text-red-500'     },
    { label: 'Comissão Shopee',   value: calc.taxaShopee, pct: (calc.taxaShopee / calc.preco) * 100, barColor: 'bg-orange-400',  textColor: 'text-orange-500'  },
    { label: `Ads (${percentualAds}%)`,  value: calc.adsVal, pct: (calc.adsVal / calc.preco) * 100,  barColor: 'bg-amber-400',   textColor: 'text-amber-500'   },
    { label: `DAS (${aliquotaDAS}%)`,    value: calc.dasVal, pct: (calc.dasVal / calc.preco) * 100,  barColor: 'bg-slate-400',   textColor: 'text-slate-500'   },
    { label: 'Lucro Líquido',     value: calc.lucro,      pct: calc.margemReal * 100,                barColor: 'bg-emerald-500', textColor: 'text-emerald-600' },
  ] : [];
  ...
  const handleSave = () => {
    ...
    const p: PrecificacaoSalva = {
      id: crypto.randomUUID(), nome: saveName.trim(), skuRef: skuRef || undefined,
      custo, embalagem, frete, comissaoShopee, taxaFixa,
      aliquotaDAS, percentualAds, margemDesejada, modo,
      preco: calc.preco, margemReal: calc.margemReal * 100, lucro: calc.lucro,
  ...
  const exportSalvasXLSX = async () => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      ...
      ...precificacoesSalvas.map((p) => [
        p.nome, p.skuRef ?? '', p.modo === 'shopee' ? 'Shopee Padrão' : 'Avançado',
        p.custo, p.embalagem, p.frete, p.aliquotaDAS, p.percentualAds, p.margemDesejada,
        p.preco, p.margemReal.toFixed(2), p.lucro.toFixed(2), p.criadoEm.slice(0, 10),
      ]),
    ]);
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {calc && (
            <button onClick={exportXLSX} className="btn-secondary flex items-center gap-1.5 text-sm">
          ...
          {calc && (
            <button
              onClick={() => {
                setSaveName(skuRef ? `${skuRef} — ${new Date().toLocaleDateString('pt-BR')}` : `Cálculo ${new Date().toLocaleDateString('pt-BR')}`);
                setShowSave((v) => !v);
              }}
      ...
      {/* Save dialog */}
      {showSave && (
        <div className="card p-4 border-shopee-200 bg-shopee-50/40">
          <p className="text-sm font-medium text-slate-700 mb-2">Nome da precificação</p>
          <div className="flex gap-2">
            <input className="input flex-1" value={saveName} onChange={(e) => setSaveName(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleSave()} autoFocus />
            <button className="btn-primary" onClick={handleSave} disabled={!saveName.trim()}>Salvar</button>
          ...
          {(['shopee', 'avancado'] as const).map((m) => (
            <button key={m} onClick={() => setModo(m)}
              className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${
                modo === m ? 'bg-white text-shopee-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        ...
        <div className="space-y-4">
          <div className="card p-5 space-y-5">
            ...
            <div>
              ...
              <select className="select" value={skuRef}
                onChange={(e) => { setSkuRef(e.target.value); loadSku(e.target.value); }}>
            ...
            <div>
              ...
              {(embalagem > 0 || frete > 0) && (
                <p className="text-xs text-slate-400 mt-2">
                  Total custo/unidade: <span className="font-semibold text-slate-600">{fmt(custo + embalagem + frete)}</span>
                </p>
              )}
            </div>
            ...
            {modo === 'shopee' ? (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-3.5 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-blue-700">Comissão Shopee: 18% + taxa por faixa de preço</span>
                  </div>
                  <button onClick={() => setShowTierInfo((v) => !v)} className="text-blue-400 hover:text-blue-600">
              ...
              <div>
                <p className="text-xs text-slate-500 mb-2 font-medium">Taxas Shopee (personalizadas)</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-slate-400 mb-1 block">Comissão (%)</label>
                    <div className="relative">
                      <input type="number" step="0.1" min="0" max="100" className="input pr-7"
            ...
            <div className="space-y-4 pt-1 border-t border-slate-50">
              <SliderField label="Alíquota DAS (%)" value={aliquotaDAS} onChange={setAliquotaDAS}
                hint="Simples Nacional — varia conforme faixa de faturamento anual" />
              <SliderField label="Marketing / Ads (%)" value={percentualAds} onChange={setPercentualAds} />
              <SliderField label="Margem Desejada (%)" value={margemDesejada} onChange={setMargemDesejada}
                min={0} max={80} />
            </div>
          </div>
          ...
          {modoReverso && (
            <div className="card p-5 border-violet-200 bg-violet-50/30 space-y-4">
              ...
              <div>
                ...
                <input type="number" step="0.01" min="0" className="input"
                  value={precoVenda || ''} placeholder="0,00"
                ...
                <div className="space-y-3 pt-2 border-t border-violet-100">
                  <div className={`text-center py-3 rounded-xl ${
                    ...
                    <p className={`text-4xl font-bold ${
                      calcReverso.margemReal >= 20 ? 'text-emerald-600'
                      : calcReverso.margemReal >= 10 ? 'text-amber-500' : 'text-red-500'
                    }`}>
                      {calcReverso.margemReal.toFixed(1)}%
                    </p>
                    <p className={`text-xs mt-1 ${calcReverso.lucro >= 0 ? 'text-slate-500' : 'text-red-500 font-medium'}`}>
        ...
        <div className="space-y-4">
          {calc ? (
            <>
              {/* Price card */}
              <div className="card p-5 bg-gradient-to-br from-shopee-500 to-shopee-600 text-white">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    {modo === 'shopee' && calc.tier && (
                      <p className="text-shopee-200 text-xs mt-2.5">
            ...
            <div className="card p-10 text-center flex flex-col items-center gap-3 text-slate-400">
              ...
              <div>
                ...
                <p className="text-xs mt-0.5">A soma das taxas + margem não pode ultrapassar 100%</p>
              </div>
            </div>
          )}
        </div>
      ...
      {calc && (
        <div className="card overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100">
            ...
            <p className="text-slate-400 text-xs mt-0.5">Impacto no lucro com diferentes preços de venda</p>
          </div>
                ...
                {simulacao.map((s, i) => {
                  ...
                  return (
                    <tr key={i} className={isIdeal ? 'bg-shopee-50' : 'hover:bg-slate-50'}>
                      <td className={`px-4 py-3 font-semibold ${isIdeal ? 'text-shopee-700' : 'text-slate-800'}`}>
                        {fmt(s.preco)}
                        {isIdeal && <span className="text-xs bg-shopee-100 text-shopee-600 px-1.5 py-0.5 rounded ml-2 font-medium">ideal</span>}
                      </td>
                      <td className={`px-4 py-3 font-medium ${s.lucro >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                        {fmt(s.lucro)}
                      </td>
                      <td className={`px-4 py-3 font-semibold ${s.margem >= 30 ? 'text-emerald-600' : s.margem >= 15 ? 'text-amber-500' : 'text-red-500'}`}>
                        {fmtPct(s.margem)}
                      </td>
        ...
        <button onClick={() => setShowSaved((v) => !v)}
          ...
          <div className="flex items-center gap-2">
            ...
            {precificacoesSalvas.length > 0 && (
              <span className="bg-shopee-100 text-shopee-600 text-xs font-semibold px-2 py-0.5 rounded-full">
          ...
          <div className="flex items-center gap-2">
            {precificacoesSalvas.length > 0 && (
              <button onClick={(e) => { e.stopPropagation(); exportSalvasXLSX(); }}
                className="flex items-center gap-1 text-xs text-slate-400 hover:text-shopee-500 px-2 py-1 rounded hover:bg-slate-100 transition-colors">
                <Download size={12} /> Exportar todas
              </button>
            )}
            {showSaved ? <ChevronUp size={16} className="text-slate-400" /> : <ChevronDown size={16} className="text-slate-400" />}
          </div>
        </button>
        ...
        {showSaved && (
          precificacoesSalvas.length === 0 ? (
            ...
            <div className="divide-y divide-slate-50">
              {precificacoesSalvas.map((p) => (
                <div key={p.id} className="flex items-center gap-3 px-5 py-3 hover:bg-slate-50 group transition-colors">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-semibold text-slate-800 truncate">{p.nome}</p>
                      {p.skuRef && (
                        <span className="text-xs bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">{p.skuRef}</span>
                      )}
                      <span className={`text-xs px-1.5 py-0.5 rounded ${p.modo === 'shopee' ? 'bg-shopee-50 text-shopee-600' : 'bg-slate-100 text-slate-500'}`}>
                        {p.modo === 'shopee' ? 'Shopee Padrão' : 'Avançado'}
                      </span>
                    </div>
                    <p className="text-xs text-slate-400 mt-0.5">
                      Preço: <span className="font-semibold text-shopee-600">{fmt(p.preco)}</span>
                      {' · '}Margem: <span className="font-semibold text-emerald-600">{p.margemReal.toFixed(1)}%</span>
                      {' · '}Custo: {fmt(p.custo)}
                  ...
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                    <button onClick={() => loadPrecificacao(p)}
                      className="text-xs text-shopee-500 hover:text-shopee-700 font-medium px-2.5 py-1.5 rounded hover:bg-shopee-50 transition-colors">
                      Carregar
                    </button>
                    <button onClick={() => { deletePrecificacao(p.id); toast('Precificação removida.', 'info'); }}
