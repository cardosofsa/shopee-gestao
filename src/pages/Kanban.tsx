...
import {
  ...
  Calendar, Flag, AlertCircle, CheckCheck,
} from 'lucide-react';
...
const COLUNAS: { id: ColunaTarefa; label: string; color: string; accent: string }[] = [
  { id: 'todo',        label: 'A Fazer',      color: 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300',     accent: 'border-slate-300 dark:border-slate-600' },
  { id: 'in_progress', label: 'Em Andamento', color: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400',  accent: 'border-amber-300 dark:border-amber-800' },
  { id: 'done',        label: 'Concluído',    color: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400', accent: 'border-emerald-300 dark:border-emerald-800' },
];
    ...
    return (
      <div ref={setNodeRef} style={style} className="bg-white dark:bg-slate-800 rounded-xl border-2 border-core-green/30 p-3 shadow-sm space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="text-[10px] text-slate-400 dark:text-slate-500 block mb-0.5">Prioridade</label>
            <select className="select text-xs py-1.5" value={editPrioridade}
          ...
          <div>
            <label className="text-[10px] text-slate-400 dark:text-slate-500 block mb-0.5">Vencimento</label>
            <input type="date" className="input text-xs py-1.5" value={editData}
  ...
  return (
    <div ref={setNodeRef} style={style}
      ...
      <div className="flex items-start gap-2">
        <button {...attributes} {...listeners} className="text-slate-300 hover:text-slate-400 mt-0.5 cursor-grab active:cursor-grabbing flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          {tarefa.descricao && (
            <p className="text-slate-500 dark:text-slate-400 text-xs mt-1 line-clamp-2">{tarefa.descricao}</p>
          )}

          <div className="flex items-center gap-3 mt-2 flex-wrap">
            <span className={`flex items-center gap-1 text-xs ${PRIOR_STYLE[tarefa.prioridade].text}`}>
...
function AddCard({ coluna, onDone }: { coluna: ColunaTarefa; onDone: () => void }) {
  ...
  const save = () => {
    ...
    addTarefa({
      ...
      coluna, posicao: Date.now(), prioridade,
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border-2 border-core-green/30 p-3 shadow-sm space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-[10px] text-slate-400 dark:text-slate-500 block mb-0.5">Prioridade</label>
          <select className="select text-xs py-1.5" value={prioridade}
        ...
        <div>
          <label className="text-[10px] text-slate-400 dark:text-slate-500 block mb-0.5">Vencimento</label>
          <input type="date" className="input text-xs py-1.5" value={dataVencimento}
...
export default function Kanban() {
  ...
  const porColuna = (col: ColunaTarefa) =>
    tarefas
      .filter((t) => t.coluna === col && (filterPrioridade === 'todas' || t.prioridade === filterPrioridade))
    ...
    <div className="p-6 h-full flex flex-col">
      ...
      <div className="mb-5">
        <div className="flex items-start justify-between flex-wrap gap-3 mb-4">
          <div>
            <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">Tarefas Operacionais</h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm">{tarefas.length} tarefas · Quadro estilo Trello</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap mb-4">
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-800 dark:text-slate-100">{abertas.length}</span>
          </div>
          <div className={`bg-white dark:bg-slate-800 border rounded-lg px-3 py-2 flex items-center gap-2 ${altaCount > 0 ? 'border-red-200 bg-red-50/40 dark:border-red-800/50 dark:bg-red-900/10' : 'border-slate-200 dark:border-slate-700'}`}>
            ...
            <span className={`text-sm font-bold ${altaCount > 0 ? 'text-red-500' : 'text-slate-400'}`}>{altaCount}</span>
          </div>
          <div className={`bg-white dark:bg-slate-800 border rounded-lg px-3 py-2 flex items-center gap-2 ${vencidasCount > 0 ? 'border-red-200 bg-red-50/40 dark:border-red-800/50 dark:bg-red-900/10' : 'border-slate-200 dark:border-slate-700'}`}>
            <AlertCircle size={11} className={vencidasCount > 0 ? 'text-red-500' : 'text-slate-400'} />
            <span className="text-xs text-slate-500 dark:text-slate-400">Vencidas</span>
            <span className={`text-sm font-bold ${vencidasCount > 0 ? 'text-red-500' : 'text-slate-400'}`}>{vencidasCount}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {([
            ...
            <button key={opt.key} onClick={() => setFilterPrioridade(opt.key)}
              ...
              <div key={col.id} className="flex flex-col w-72 flex-shrink-0">
                ...
                <div className="flex-1 space-y-2 min-h-24">
                  ...
                  {items.length === 0 && addingIn !== col.id && (
                    <div className={`border-2 border-dashed rounded-xl h-20 flex items-center justify-center ${col.accent} border-opacity-50`}>
