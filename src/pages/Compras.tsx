...
import {
  ShoppingBag, TrendingUp, TrendingDown, ArrowRight,
  Package, BarChart2, ChevronDown,
} from 'lucide-react';
import {
...
function isoToMes(data: string) { return data.slice(0, 7); }
  ...
  return (
    <div className="card p-5 flex items-start gap-4">
      ...
      <div className="min-w-0">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-0.5">{label}</p>
        <p className="text-xl font-bold text-slate-900 dark:text-slate-100 truncate">{value}</p>
        {sub && <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{sub}</p>}
        {delta != null && (
          <div className={`flex items-center gap-1 mt-1 text-[11px] font-medium ${delta >= 0 ? 'text-red-500' : 'text-green-600'}`}>
...
export default function Compras() {
  ...
  const dataCorte = useMemo(
    () => periodo > 0 ? subMonths(periodo) : '',
    [periodo],
  );

  const compras = useMemo(
    () => periodo > 0
      ? comprasAll.filter((c) => isoToMes(c.data) >= dataCorte)
      : comprasAll,
  ...
  const cmvMes     = useMemo(() => comprasAll.filter((c) => isoToMes(c.data) === mesAtual).reduce((s, c) => s + c.custoTotal, 0), [comprasAll, mesAtual]);
  const cmvMesAnt  = useMemo(() => comprasAll.filter((c) => isoToMes(c.data) === mesAnt  ).reduce((s, c) => s + c.custoTotal, 0), [comprasAll, mesAnt]);
  ...
  const fornecedorLider = useMemo(() => {
    ...
    compras.forEach((c) => { map[c.fornecedor || '(sem fornecedor)'] = (map[c.fornecedor || '(sem fornecedor)'] ?? 0) + c.custoTotal; });
    ...
    <div className="flex-1 p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Análise de Compras</h1>
            <p className="text-xs text-slate-400 dark:text-slate-500">CMV, evolução de custo e gasto por fornecedor</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link
            to="/estoque"
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400 hover:text-core-green transition-colors border border-slate-200 dark:border-slate-700 rounded-lg"
          >
            Gerenciar compras <ArrowRight size={12} />
          </Link>
        </div>
      </div>
      ...
      <div className="card p-5">
        <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-4">CMV Mensal</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Por fornecedor */}
        <div className="card p-5">
          <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-4">Gasto por Fornecedor</h2>
                ...
                return (
                  <div key={name}>
                    <div className="flex items-center justify-between text-xs mb-1">
                      <span className="text-slate-600 dark:text-slate-300 truncate max-w-[60%]">{name}</span>
                      <span className="font-medium text-slate-700 dark:text-slate-200 tabular-nums">{fmt(valor)}</span>
                    </div>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Evolução do Custo Unitário</h2>
            {custoTrend != null && (
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                ...
                {custoTrend > 0 ? '+' : ''}{custoTrend.toFixed(1)}% total
              </span>
            )}
          </div>
          ...
          {skusDisponiveis.length > 0 && (
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500 truncate max-w-[120px]">{produtoNome}</span>
            </div>
          )}
      ...
      <div className="card">
        <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Compras Recentes</h2>
          <Link
            to="/estoque"
            className="text-xs text-core-green font-medium flex items-center gap-1 hover:underline"
          >
            Ver e gerenciar todas <ArrowRight size={11} />
          </Link>
        </div>
        {recentes.length === 0 ? (
          <EmptyState
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/60 text-slate-400 dark:text-slate-500 text-xs uppercase tracking-wide">
                    {['Data', 'SKU', 'Produto', 'Qtd', 'Custo Unit.', 'Custo Total', 'Fornecedor', 'Pagamento'].map((h) => (
                      ...
                      <td className="px-4 py-3 text-xs">
                        {c.pagamento ? (
                          <span className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-[10px] font-medium">
                            {c.pagamento}{c.parcelas > 1 ? ` ${c.parcelas}×` : ''}
                          </span>
            ...
            {comprasAll.length > 5 && (
              <div className="px-5 py-3 border-t border-slate-50 dark:border-slate-800">
                ...
                >
                  ...
                  <ChevronDown size={11} className={`transition-transform ${showTop ? 'rotate-180' : ''}`} />
                </button>
              </div>
            )}
          </>
