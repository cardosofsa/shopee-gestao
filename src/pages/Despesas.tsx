...
import {
  Plus, Trash2, Pencil, ChevronLeft, ChevronRight, Receipt,
  TrendingUp, TrendingDown, X, Download, Target, Check,
} from 'lucide-react';
import {
...
const CHART_COLORS = [
  C.primary, C.blue, C.amber, C.red, '#8b5cf6',
  '#06b6d4', '#ec4899', C.orange, C.secondary, '#64748b',
];
...
function catColor(cat: string, all: string[]) {
  ...
  return { badge: BADGE_PALETTE[idx % BADGE_PALETTE.length], chart: CHART_COLORS[idx % CHART_COLORS.length] };
}
...
const monthShort = (mes: string) =>
  new Date(mes + '-02').toLocaleString('pt-BR', { month: 'short', year: '2-digit' }).replace('.', '');

// ─── DespesaModal ─────────────────────────────────────────────────────────────

function DespesaModal({ title, initial, categorias, onSave, onClose }: {
  title: string;
    ...
    <Modal onClose={onClose} maxWidth="max-w-md">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <input className="input" placeholder="Ex: Caixas de papelão 20×20cm" value={form.descricao} onChange={f('descricao')} />
          </div>
          <div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3">
          ...
          <button className="btn-primary" disabled={!valid} onClick={() => valid && onSave(form)}>Salvar</button>
        </div>
    </Modal>
  );
}
...
function OrcamentoRow({ cat, gasto, orcamento, onSave }: {
  cat: string; gasto: number; orcamento: number; onSave: (v: number) => void;
}) {
    ...
    <div className="flex items-center gap-3">
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-medium text-slate-700 dark:text-slate-200 truncate">{cat}</span>
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className="text-xs font-bold text-slate-700 dark:text-slate-200">{fmt(gasto)}</span>
            {editing ? (
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  autoFocus
                />
                <button onClick={save} className="text-core-green"><Check size={11} /></button>
              </div>
            ) : (
              <button
              ...
              >
                {orcamento > 0 ? <><span className="text-slate-300 dark:text-slate-600">/</span> {fmt(orcamento)}</> : <Target size={10} />}
              </button>
            )}
          </div>
        </div>
        ...
        {overBudget && (
          <p className="text-[10px] text-red-500 mt-0.5">Orçamento excedido em {fmt(gasto - orcamento)}</p>
        )}
        {nearLimit && (
...
export default function Despesas() {
  ...
  const prevMonth = () => { const [y, m] = mesFiltro.split('-').map(Number); setMesFiltro(new Date(y, m - 2, 1).toISOString().slice(0, 7)); };
  const nextMonth = () => { const [y, m] = mesFiltro.split('-').map(Number); setMesFiltro(new Date(y, m, 1).toISOString().slice(0, 7)); };
  ...
  const doMes         = useMemo(() => despesas.filter((d) => d.data.startsWith(mesFiltro)), [despesas, mesFiltro]);
  const doMesAnterior = useMemo(() => despesas.filter((d) => d.data.startsWith(prevMes)),   [despesas, prevMes]);
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm capitalize">{monthLabel(mesFiltro)}</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2 py-1">
            <button onClick={prevMonth} className="p-1 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 rounded transition-colors"><ChevronLeft size={15} /></button>
            <span className="text-sm font-medium text-slate-700 dark:text-slate-200 capitalize px-1 min-w-32 text-center">{monthLabel(mesFiltro)}</span>
            <button onClick={nextMonth} disabled={isCurrentMonth} className="p-1 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 rounded transition-colors disabled:opacity-30 disabled:cursor-not-allowed"><ChevronRight size={15} /></button>
          </div>
          {doMes.length > 0 && (
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <div className="card p-4">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1">Total do Mês</p>
          <p className="text-2xl font-bold text-red-500">{fmt(totalMes)}</p>
          {totalMesAnt > 0 && (
            <div className={`flex items-center gap-1 mt-1 text-xs font-medium ${delta > 0 ? 'text-red-500' : 'text-emerald-600'}`}>
              {delta > 0 ? <TrendingUp size={11} /> : <TrendingDown size={11} />}
              {delta > 0 ? '+' : ''}{deltaPct.toFixed(0)}% vs mês ant.
            </div>
          )}
        </div>
        <div className="card p-4">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1">Lançamentos</p>
          <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">{doMes.length}</p>
          <p className="text-xs text-slate-400 mt-1">{porCategoria.length} categoria{porCategoria.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="card p-4">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1">Maior Categoria</p>
          <p className="text-base font-bold text-slate-800 dark:text-slate-100 truncate">{porCategoria[0]?.[0] ?? '—'}</p>
          <p className="text-xs text-red-400 mt-1">{porCategoria[0] ? fmt(porCategoria[0][1]) : '—'}</p>
        </div>
        <div className="card p-4">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1">Orçamento Total</p>
          <p className={`text-2xl font-bold ${totalOrcamento > 0 && totalMes > totalOrcamento ? 'text-red-500' : 'text-slate-900 dark:text-slate-100'}`}>
            {totalOrcamento > 0 ? fmt(totalOrcamento) : '—'}
          </p>
          {totalOrcamento > 0 && (
            <p className="text-xs text-slate-400 mt-1">{((totalMes / totalOrcamento) * 100).toFixed(0)}% usado</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        {catChartData.length > 0 && (
          <div className="card p-5">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-4">Gastos por Categoria</p>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={catChartData} layout="vertical" margin={{ top: 0, right: 60, left: 0, bottom: 0 }}>
                ...
                <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: C.slate }} axisLine={false} tickLine={false} width={72} />
                <Tooltip
                  formatter={(v: any, _: any, p: any) => [fmt(Number(v)), p.payload.fullName]}
                  contentStyle={{ fontSize: 12 }}
                  cursor={{ fill: 'rgba(148,163,184,0.06)' }}
                />
                <Bar dataKey="valor" radius={[0, 4, 4, 0]}>
                  {catChartData.map((d) => <Cell key={d.name} fill={d.fill} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        {/* Tendência 6 meses */}
        <div className="card p-5">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-4">Tendência — Últimos 6 Meses</p>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={trendData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: C.slate }} tickFormatter={(v) => `R$${(v / 1000).toFixed(0)}k`} axisLine={false} tickLine={false} width={48} />
              <Tooltip formatter={(v: any) => [fmt(Number(v)), 'Total Despesas']} contentStyle={{ fontSize: 12 }} />
              <Area type="monotone" dataKey="total" name="Total" stroke={C.red} strokeWidth={2} fill="url(#gradDesp)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      {categorias.length > 0 && (
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Orçamento por Categoria</h2>
            <span className="text-xs text-slate-400 dark:text-slate-500">· clique no valor para editar</span>
          </div>
          ...
          {categorias.every((cat) => {
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 text-center py-4">Nenhuma despesa neste mês.</p>
          )}
        </div>
      )}
              ...
              {filtradas.length === 0 ? (
                ...
                <tr key={d.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${catColor(d.categoria, categorias).badge}`}>
                      {d.categoria}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    {!d.compraRef ? (
                      <div className="flex items-center gap-2">
                        <button onClick={() => setEditTarget(d)} className="text-slate-300 hover:text-core-green transition-colors" title="Editar">
                          <Pencil size={13} />
                        </button>
            ...
            {filtradas.length > 0 && (
              <tfoot>
                <tr className="bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 font-semibold">
                  <td colSpan={4} className="px-4 py-3 text-slate-700 dark:text-slate-200 text-xs uppercase tracking-wide">Total filtrado</td>
