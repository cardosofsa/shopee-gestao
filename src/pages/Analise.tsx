...
import {
  CalendarSearch, TrendingUp, TrendingDown, GitCompare, Download,
} from 'lucide-react';
import {
...
function startOfMonth() { return new Date().toISOString().slice(0, 7) + '-01'; }
...
function shiftPeriod(from: string, to: string): { from: string; to: string } {
  ...
  const f = new Date(from); f.setDate(f.getDate() - d);
  const t = new Date(to);   t.setDate(t.getDate() - d);
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 px-3 py-2.5 text-xs space-y-1">
      ...
      {payload.map((p: any) => (
        <div key={p.name} className="flex items-center gap-2">
          ...
          <span className="font-medium text-slate-700 dark:text-slate-200">{fmt(Number(p.value))}</span>
        </div>
      ))}
    </div>
  );
...
function computeMetrics(
  ...
  return { receita, cmv, lucroOp, taxas, despesas, lucroLiq, margem, margemLiq, pedidosCount, ticket, unidades, devCount, taxaDev, ps };
}
...
function KPI({
  label, a, b, fmt: f = fmt, positive = true,
}: {
  label: string; a: number; b?: number; fmt?: (v: number) => string; positive?: boolean;
}) {
  ...
  return (
    <div className="card p-4">
      ...
      {delta != null && (
        <div className={`flex items-center gap-1 mt-1 text-[11px] font-medium ${good ? 'text-green-600' : 'text-red-500'}`}>
...
function DRELine({ label, a, b, negative, indent }: { label: string; a: number; b?: number; negative?: boolean; indent?: boolean }) {
  const delta = b != null && b !== 0 ? ((a - b) / Math.abs(b)) * 100 : null;
  const up = delta != null && delta > 0;
  return (
    <div className={`flex items-center justify-between py-2 text-sm ${
      indent ? 'pl-4 text-slate-500 dark:text-slate-400' : 'font-medium text-slate-700 dark:text-slate-200'
    } border-b border-slate-50 dark:border-slate-800`}>
      <span>{label}</span>
      <div className="flex items-center gap-3">
        {b != null && (
          <span className="text-xs text-slate-400 dark:text-slate-500 tabular-nums w-24 text-right">{fmt(b)}</span>
        )}
        <span className={`tabular-nums w-24 text-right ${negative && a > 0 ? 'text-red-500' : ''}`}>{fmt(a)}</span>
        {delta != null && (
          <span className={`text-[10px] w-16 text-right font-medium ${up ? 'text-green-600' : 'text-red-500'}`}>
...
const PRESETS = [
  ...
  { label: 'Últimos 7d',    from: () => { const d = new Date(); d.setDate(d.getDate()-6); return d.toISOString().slice(0,10); }, to: () => today() },
  { label: 'Últimos 30d',   from: () => { const d = new Date(); d.setDate(d.getDate()-29); return d.toISOString().slice(0,10); }, to: () => today() },
  { label: 'Últimos 90d',   from: () => { const d = new Date(); d.setDate(d.getDate()-89); return d.toISOString().slice(0,10); }, to: () => today() },
];
...
export default function Analise() {
  ...
  const ranking = useMemo(
    () => getRankingProdutos(mA.ps).slice(0, 8),
    [mA.ps],
  );

  const handleExport = async () => {
    const headers = ['Data', 'Nº Pedido', 'Status', 'Loja', 'SKU', 'Produto', 'Qtd', 'Receita', 'CMV', 'Lucro Op.'];
    const rows = mA.ps.map((p) => [p.data, p.numeroPedido, p.status, p.loja, p.sku, p.produto, xlsxNum(p.quantidade), xlsxNum(p.receita), xlsxNum(p.custoTotal), xlsxNum(p.lucroOperacional)]);
    ...
    <div className="flex-1 p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Análise por Período</h1>
            <p className="text-xs text-slate-400 dark:text-slate-500">P&L completo para qualquer intervalo de datas</p>
          </div>
        </div>
        <button
      ...
      <div className="card p-4 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <label className="text-xs font-medium text-slate-600 dark:text-slate-300">Período A</label>
          </div>
          <input
            type="date"
            value={dateFrom}
          ...
          <input
            ...
            className="input text-xs py-1.5"
          />
          <span className="text-xs text-slate-400 dark:text-slate-500">{dias} dia{dias !== 1 ? 's' : ''}</span>
        ...
        {compare && (
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <label className="text-xs font-medium text-slate-500 dark:text-slate-400">Período B</label>
            </div>
            <input
              type="date"
              value={dateFrom2}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3">
        ...
        <KPI label="Devoluções"    a={mA.devCount}    b={mB?.devCount}    fmt={(v) => String(v)} positive={false} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          {chartA.length === 0 || chartA.every((d) => d.Receita === 0) ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={chartA} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="Receita" stroke={C.primary} fill="url(#gR)" strokeWidth={2} dot={false} />
                <Area type="monotone" dataKey="Lucro"   stroke={C.secondary} fill="url(#gL)" strokeWidth={1.5} dot={false} strokeDasharray="4 2" />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="card p-5">
          <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-3">
            Demonstrativo
            {mB && <span className="ml-2 text-xs font-normal text-slate-400 dark:text-slate-500">A / B</span>}
          </h2>
          <div className="space-y-0">
            ...
            <DRELine label="(−) Despesas Op."       a={mA.despesas}  b={mB?.despesas}  negative indent />
            ...
            <div className="flex items-center justify-between pt-3 text-xs text-slate-500 dark:text-slate-400">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`w-16 text-right font-semibold ${mA.margem >= 20 ? 'text-green-600' : mA.margem >= 10 ? 'text-amber-600' : 'text-red-500'}`}>
                  {fmtPct(mA.margem)}
                </span>
              </div>
            </div>
      ...
      {mB && (
        <div className="card p-5">
          <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-4">Comparativo A vs B</h2>
          <ResponsiveContainer width="100%" height={200}>
            ...
            >
              ...
              <Tooltip content={<ChartTooltip />} />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <Bar dataKey="A" name={`A: ${fmtDate(dateFrom)}→${fmtDate(dateTo)}`} fill={C.primary} radius={[4,4,0,0]} />
              <Bar dataKey="B" name={`B: ${fmtDate(dateFrom2)}→${fmtDate(dateTo2)}`} fill={C.slate} radius={[4,4,0,0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/60 text-slate-400 dark:text-slate-500 uppercase tracking-wide">
                  {['SKU', 'Produto', 'Pedidos', 'Unid.', 'Receita', 'Lucro Op.', 'Margem', 'Curva'].map((h) => (
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                {ranking.map((r) => (
                  <tr key={r.sku} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-4 py-3 font-mono text-core-green">{r.sku}</td>
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-200 max-w-[200px] truncate">{r.produto}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300 text-center">{r.pedidos}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300 text-center">{r.unidades}</td>
                    <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-100 tabular-nums">{fmt(r.receita)}</td>
                    <td className="px-4 py-3 text-green-600 dark:text-green-400 tabular-nums">{fmt(r.lucroOperacional)}</td>
                    <td className="px-4 py-3">
                      <span className={r.margem >= 20 ? 'text-green-600 font-semibold' : r.margem >= 10 ? 'text-amber-600' : 'text-red-500'}>
                        {r.margem.toFixed(1)}%
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        r.curvaABC === 'A' ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300' :
