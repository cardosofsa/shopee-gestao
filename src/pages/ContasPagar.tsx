...
import {
  Plus, X, CheckCircle2, Clock, AlertTriangle, CreditCard,
  Pencil, Trash2, ChevronDown, RefreshCw, DollarSign,
...
const CATEGORIAS = ['DAS / Imposto', 'Fornecedor', 'Aluguel', 'Marketing', 'Embalagem', 'Frete', 'Serviço', 'Outro'];
const today = () => new Date().toISOString().slice(0, 10);

function diasAte(vencimento: string): number {
  const diff = new Date(vencimento + 'T00:00:00').getTime() - new Date(today() + 'T00:00:00').getTime();
...
const STATUS_CFG = {
  pago:     { label: 'Pago',     cls: 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-800', dot: 'bg-emerald-400' },
  vencido:  { label: 'Vencida',  cls: 'bg-red-50 text-red-700 border-red-200 dark:bg-red-950/20 dark:text-red-400 dark:border-red-800', dot: 'bg-red-500 animate-pulse' },
  urgente:  { label: 'Urgente',  cls: 'bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-950/20 dark:text-orange-400 dark:border-orange-800', dot: 'bg-orange-400 animate-pulse' },
  pendente: { label: 'Pendente', cls: 'bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-700/30 dark:text-slate-300 dark:border-slate-600', dot: 'bg-slate-400' },
} as const;
...
const EMPTY: Omit<ContaPagar, 'id'> = {
  descricao: '', categoria: 'Fornecedor', valor: 0,
  vencimento: '', status: 'pendente', recorrente: false,
  loja: 'Ambas', observacoes: '',
};

function ContaForm({ initial, lojas, onSave, onClose }: {
  initial?: ContaPagar;
  lojas: string[];
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input className="input" placeholder="Ex: DAS Julho, Fornecedor Binho…"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs text-slate-500 dark:text-slate-400 mb-1 block">Vencimento *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div className="relative">
                <select className="input appearance-none pr-8"
                  value={form.loja} onChange={(e) => set('loja', e.target.value as ContaPagar['loja'])}>
          ...
          <div>
            ...
            <input className="input" placeholder="Notas opcionais…"
              value={form.observacoes ?? ''} onChange={(e) => set('observacoes', e.target.value)} />
          </div>
          <label className="flex items-center gap-2.5 cursor-pointer select-none">
            <div className={`w-8 h-4.5 rounded-full relative transition-colors ${form.recorrente ? 'bg-core-green' : 'bg-slate-200 dark:bg-slate-600'}`}
              onClick={() => set('recorrente', !form.recorrente)}>
              <div className={`absolute top-0.5 w-3.5 h-3.5 rounded-full bg-white shadow transition-transform ${form.recorrente ? 'translate-x-4' : 'translate-x-0.5'}`} />
            </div>
        ...
        <div className="px-6 pb-5 flex gap-3">
          ...
          <button className="btn-primary flex-1 justify-center" disabled={!valid} onClick={() => valid && onSave(form)}>
...
function ConfirmDelete({ descricao, onConfirm, onCancel }: { descricao: string; onConfirm: () => void; onCancel: () => void }) {
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onCancel}>
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <h3 className="font-semibold text-slate-900 dark:text-slate-100 text-center mb-1">Excluir conta?</h3>
        ...
        <div className="flex gap-3">
          ...
          <button className="flex-1 py-2 rounded-xl text-sm font-medium text-white bg-red-500 hover:bg-red-600 transition-colors flex items-center justify-center gap-1.5" onClick={onConfirm}>
...
export default function ContasPagar() {
  ...
  const vencidas    = useMemo(() => pendentes.filter((c) => c.vencimento < todayStr), [pendentes, todayStr]);
  const proximas7   = useMemo(() => pendentes.filter((c) => { const d = diasAte(c.vencimento); return d >= 0 && d <= 7; }), [pendentes]);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">Gestão de obrigações financeiras e projeção de saldo</p>
        </div>
      ...
      {/* Saldo Futuro */}
      <div className={`card p-4 flex items-center gap-4 flex-wrap ${saldoFuturo >= 0 ? 'border-l-4 border-l-emerald-400' : 'border-l-4 border-l-red-400'}`}>
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${saldoFuturo >= 0 ? 'bg-emerald-50 dark:bg-emerald-950/20' : 'bg-red-50 dark:bg-red-950/20'}`}>
          {saldoFuturo >= 0 ? <DollarSign size={18} className="text-emerald-500" /> : <TrendingDown size={18} className="text-red-500" />}
        </div>
        <div className="flex-1">
          <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Saldo Futuro Estimado</p>
          <p className={`text-2xl font-bold mt-0.5 ${saldoFuturo >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500'}`}>
            {fmt(saldoFuturo)}
          </p>
        </div>
        <div className="flex gap-6 text-sm text-slate-500 dark:text-slate-400 flex-wrap">
          ...
          <div className="text-right">
            ...
            <p className="font-semibold text-slate-600 dark:text-slate-300">− {fmt(totalPendente)}</p>
          </div>
        </div>
      </div>
        ...
        {(['pendentes', 'pagas', 'todas'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium capitalize transition-colors ${
              tab === t ? 'bg-core-green text-white' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 dark:text-slate-400'
            }`}>
            {t}
          </button>
        ))}
        {categorias.length > 0 && (
          <div className="flex items-center gap-1.5 ml-2">
            ...
            {[null, ...categorias].map((cat) => (
              <button key={cat ?? '_all'} onClick={() => setCatFiltro(cat)}
                className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${
                  catFiltro === cat ? 'bg-slate-800 dark:bg-slate-200 text-white dark:text-slate-900' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'
                }`}>
                {cat ?? 'Todas'}
              </button>
            ))}
      ...
      {listaFiltrada.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          {contas.length === 0 && (
            <button className="mt-3 text-xs text-core-green hover:underline" onClick={() => setShowForm(true)}>
        ...
        <div className="card overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700">
                  {['Descrição', 'Categoria', 'Valor', 'Vencimento', 'Status', 'Loja', ''].map((h) => (
                    <th key={h} className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700/40">
                {listaFiltrada.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className={`transition-colors ${urg ? 'bg-red-50/30 dark:bg-red-950/5 hover:bg-red-50/60 dark:hover:bg-red-950/10' : 'hover:bg-slate-50 dark:hover:bg-slate-700/20'}`}>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          {c.recorrente && <span title="Recorrente"><RefreshCw size={11} className="text-slate-300 dark:text-slate-600 flex-shrink-0" /></span>}
                          <span className="font-medium text-slate-800 dark:text-slate-100 truncate max-w-48">{c.descricao}</span>
                        </div>
                        {c.observacoes && <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5 truncate max-w-48">{c.observacoes}</p>}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        ...
                        {c.status === 'pendente' && (
                          <p className={`text-[11px] mt-0.5 ${se === 'vencido' ? 'text-red-500' : se === 'urgente' ? 'text-orange-500' : 'text-slate-400'}`}>
                            {se === 'vencido' ? `${Math.abs(d)}d em atraso` : d === 0 ? 'vence hoje' : `${d}d restantes`}
                          </p>
                        )}
                      ...
                      <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap">{c.loja}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          {c.status === 'pendente' && (
                            <button
                          ...
                          <button onClick={() => setEditItem(c)} className="text-slate-300 hover:text-core-green transition-colors" title="Editar">
                            <Pencil size={13} />
                          </button>
                          <button onClick={() => setDelItem(c)} className="text-slate-300 hover:text-red-400 transition-colors" title="Excluir">
                            <Trash2 size={13} />
                          </button>
                        </div>
                      </td>
          ...
          <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex justify-between text-xs text-slate-400 dark:text-slate-500">
            <span>{listaFiltrada.length} conta{listaFiltrada.length !== 1 ? 's' : ''}</span>
      ...
      {editItem && (
        <ContaForm initial={editItem} lojas={lojas} onSave={handleSave} onClose={() => setEditItem(null)} />
      )}
      {delItem && (
        <ConfirmDelete
