...
import {
  Waves, Plus, X, TrendingUp, TrendingDown, AlertTriangle,
  ArrowUpRight, ArrowDownRight, CheckCircle2, Trash2,
} from 'lucide-react';
import {
...
function KCard({ label, value, sub, pos, icon: Icon }: {
  label: string; value: string; sub?: string; pos?: boolean; icon: React.ElementType;
}) {
  const color = pos === undefined ? 'text-slate-800 dark:text-slate-100'
  ...
  return (
    <div className="card p-4">
      <div className="flex items-center justify-between mb-2">
        ...
        <Icon size={13} className={pos === undefined ? 'text-slate-400' : pos ? 'text-emerald-500' : 'text-red-400'} />
      </div>
...
const ChartTip = ({ active, payload, label }: any) => {
  ...
  const ent   = payload.find((p: any) => p.dataKey === 'entradas')?.value ?? 0;
  const sai   = payload.find((p: any) => p.dataKey === 'saidas')?.value ?? 0;
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 shadow-lg text-xs space-y-1 min-w-40">
      ...
      <p className="text-emerald-600 flex items-center gap-1"><ArrowUpRight size={10} /> Entradas: {fmt(ent)}</p>
      <p className="text-red-500 flex items-center gap-1"><ArrowDownRight size={10} /> Saídas: {fmt(sai)}</p>
      <p className={`font-bold ${saldo >= 0 ? 'text-core-green' : 'text-red-500'}`}>Saldo: {fmt(saldo)}</p>
    </div>
  );
};
...
function NovoLancamento({ onSave, onClose }: {
  onSave: (l: LancamentoManual) => void; onClose: () => void;
}) {
  const today = new Date().toISOString().slice(0, 10);
  const [form, setForm] = useState({ descricao: '', valor: '', tipo: 'entrada' as 'entrada' | 'saida', data: today });
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md">
        <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
          <h3 className="font-semibold text-slate-800 dark:text-slate-100 text-sm">Novo Lançamento</h3>
        ...
        <div className="p-5 space-y-3">
          ...
          <div>
            ...
            <input value={form.descricao} onChange={f('descricao')} placeholder="Ex: Pagamento cliente" className="input-field w-full" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Valor (R$)</label>
            <input type="number" min="0" step="0.01" value={form.valor} onChange={f('valor')} placeholder="0,00" className="input-field w-full" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Data</label>
            <input type="date" value={form.data} onChange={f('data')} className="input-field w-full" />
          </div>
        </div>
        <div className="px-5 pb-4 flex gap-2 justify-end">
          <button onClick={onClose} className="btn-secondary text-sm px-4 py-2 rounded-xl">Cancelar</button>
          <button onClick={save} className="btn-primary text-sm px-4 py-2 rounded-xl">Salvar</button>
        </div>
      </div>
    </div>
  );
...
export default function FluxoCaixa() {
  ...
  const pedidos30d = useMemo(() => {
    ...
    return (lojaFiltro ? pedidosAll.filter((p) => p.loja === lojaFiltro) : pedidosAll)
      .filter((p) => p.data >= cutoff && (p.status === 'Concluído' || p.status === 'Enviado'));
  ...
  const timeline = useMemo(() => {
    const days: {
      iso: string; label: string; entradas: number; saidas: number; saldo: number;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">Projeção das entradas e saídas dos próximos dias</p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 p-4 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-xl">
          ...
          <div>
            ...
            <p className="text-xs text-red-600/80 dark:text-red-400/70 mt-0.5">
              Mínimo previsto: {fmt(minimoSaldo)} em {diasNeg} dia{diasNeg !== 1 ? 's' : ''} negativos. Revise as contas a pagar ou antecipe receitas.
            </p>
          </div>
        </div>
      )}
      ...
      {saldoFinal >= 0 && diasAteNeg === null && (
        <div className="flex items-center gap-3 p-4 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-xl">
          ...
          <p className="text-sm font-medium text-emerald-700 dark:text-emerald-400">
            Fluxo positivo nos próximos {horizonte} dias — saldo projetado: <span className="font-bold">{fmt(saldoFinal)}</span>
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <KCard label="Total Entradas" value={fmt(totalEntradas)} sub={`próximos ${horizonte}d`} pos={true}  icon={ArrowUpRight} />
        <KCard label="Total Saídas"   value={fmt(totalSaidas)}   sub={`próximos ${horizonte}d`} pos={false} icon={ArrowDownRight} />
        <KCard
          label="Saldo Projetado"
          value={fmt(saldoFinal)}
          sub={saldoFinal >= 0 ? `positivo ao fim de ${horizonte}d` : `negativo ao fim de ${horizonte}d`}
          pos={saldoFinal >= 0}
      ...
      <div className="card p-5">
        ...
        <ResponsiveContainer width="100%" height={240}>
          <AreaChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="label" tick={{ fontSize: 9, fill: C.slate }} axisLine={false} tickLine={false}
              interval={Math.floor(chartData.length / 8)} />
            <YAxis tick={{ fontSize: 9, fill: C.slate }} tickFormatter={(v) => `R$${(v / 1000).toFixed(0)}k`} axisLine={false} tickLine={false} width={46} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <div className="card p-5">
          ...
          {proximasSaidas.length === 0 ? (
            <p className="text-sm text-slate-400 dark:text-slate-500 py-4 text-center">Nenhuma conta pendente nos próximos 14 dias</p>
          ) : (
            <div className="space-y-1.5">
              {proximasSaidas.map((c) => {
                const diff = Math.ceil((new Date(c.vencimento + 'T12:00:00').getTime() - hoje.getTime()) / 86_400_000);
                return (
                  <div key={c.id} className="flex items-center justify-between py-1.5 border-b border-slate-50 dark:border-slate-700/50 last:border-0">
                    <div className="min-w-0 mr-2">
                      <p className="text-sm text-slate-700 dark:text-slate-200 truncate">{c.descricao}</p>
                      <p className={`text-xs ${diff < 0 ? 'text-red-500' : diff <= 2 ? 'text-amber-500' : 'text-slate-400'}`}>
                        {diff < 0 ? `Vencida há ${Math.abs(diff)}d` : diff === 0 ? 'Vence hoje' : `Em ${diff} dia${diff !== 1 ? 's' : ''}`}
                      </p>
                    </div>
                    <span className="text-sm font-semibold text-red-500 font-mono flex-shrink-0">{fmt(c.valor)}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShowForm(true)} className="text-xs text-core-green hover:underline flex items-center gap-1">
              <Plus size={11} /> Adicionar
            </button>
          </div>
          {manuais.length === 0 ? (
            <p className="text-sm text-slate-400 dark:text-slate-500 py-4 text-center">Nenhum lançamento manual.<br />Use para planejar receitas ou despesas pontuais.</p>
          ) : (
            <div className="space-y-1.5">
              {manuais.map((m) => (
                <div key={m.id} className="flex items-center justify-between py-1.5 border-b border-slate-50 dark:border-slate-700/50 last:border-0">
                  <div className="min-w-0 mr-2">
                    <p className="text-sm text-slate-700 dark:text-slate-200 truncate">{m.descricao}</p>
                    <p className="text-xs text-slate-400">{new Date(m.data + 'T12:00:00').toLocaleDateString('pt-BR')}</p>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className={`text-sm font-semibold font-mono ${m.tipo === 'entrada' ? 'text-emerald-600' : 'text-red-500'}`}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
              {timeline.slice(0, 14).map((d) => {
                ...
                return (
                  <tr key={d.iso} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">
                    <td className="px-4 py-2.5 text-slate-600 dark:text-slate-300 whitespace-nowrap">{dayLabel(d.iso)}</td>
                    <td className="px-4 py-2.5 text-emerald-600 font-medium font-mono">{fmt(d.entradas)}</td>
                    <td className="px-4 py-2.5 text-red-500 font-medium font-mono">{fmt(d.saidas)}</td>
                    <td className={`px-4 py-2.5 font-medium font-mono ${res >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                      {res >= 0 ? '+' : ''}{fmt(res)}
                    </td>
                    <td className={`px-4 py-2.5 font-bold font-mono ${d.saldo >= 0 ? 'text-slate-800 dark:text-slate-100' : 'text-red-600'}`}>
                      {fmt(d.saldo)}
                    </td>
                  </tr>
                );
      ...
      <p className="text-xs text-slate-400 dark:text-slate-600 text-center">
        Entradas: média diária dos últimos 30 dias · Saídas: média de despesas dos últimos 60 dias + contas a pagar programadas · Lançamentos manuais são session-only e não persistem.
      </p>

      {showForm && <NovoLancamento onSave={(l) => setManuais((p) => [...p, l])} onClose={() => setShowForm(false)} />}
