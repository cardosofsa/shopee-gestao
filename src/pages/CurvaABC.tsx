  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 px-3 py-2 text-xs">
      ...
      <p className="text-slate-500 dark:text-slate-400">
        {fmt(value)} · {fmtPct(payload[0].payload.pct)}
      </p>
  ...
  return (
    <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors border-b border-slate-50 dark:border-slate-800">
      <td className="px-4 py-2.5">
        {cfg ? (
          <span
            className={`inline-flex items-center justify-center w-6 h-6 rounded-md text-xs font-bold ${cfg.badge}`}
          >
            {tier}
          </span>
        ) : (
      ...
      <td className="px-4 py-2.5 text-slate-700 dark:text-slate-200 max-w-[200px] truncate text-xs">
        {p.produto}
      </td>
      <td className="px-4 py-2.5 text-slate-600 dark:text-slate-300 text-center text-xs">
        {p.pedidos}
      </td>
      <td className="px-4 py-2.5 text-slate-600 dark:text-slate-300 text-center text-xs">
        {p.unidades}
      </td>
      <td className="px-4 py-2.5 font-medium text-slate-800 dark:text-slate-100 tabular-nums text-xs text-right">
        {fmt(p.receita)}
      </td>
      <td className="px-4 py-2.5 text-green-600 dark:text-green-400 tabular-nums text-xs text-right">
      ...
      <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400 tabular-nums text-xs text-right">
        {fmtPct(p.percentReceita)}
      </td>
      <td className="px-4 py-2.5 text-slate-400 dark:text-slate-500 tabular-nums text-xs text-right">
        {fmtPct(acumPct)}%
      </td>
    </tr>
  );
...
export function CurvaABCContent({ embedded = false }: { embedded?: boolean }) {
  ...
  const tierStats = useMemo(() => {
    const s = {
      A: { qtd: 0, receita: 0, lucro: 0 },
      B: { qtd: 0, receita: 0, lucro: 0 },
      C: { qtd: 0, receita: 0, lucro: 0 },
    };
    ranking.forEach((r) => {
  ...
  const handleExport = async () => {
    ...
    const rows = sorted.map((r) => [
      ...
      xlsxNum(r.unidades),
      xlsxNum(r.receita),
      xlsxNum(r.lucroOperacional),
      xlsxNum(r.margem),
      xlsxNum(r.percentReceita),
    ]);
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="card p-5 flex flex-col items-center justify-center">
              <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2 self-start">
                ...
                <div key={tier} className={`p-4 rounded-xl border ${cfg.border} ${cfg.bg}`}>
                  ...
                  <ul className="space-y-1">
                    {cfg.actions.map((a, i) => (
                      ...
                      >
                        <span className={`mt-0.5 shrink-0 text-[8px] font-bold ${cfg.text}`}>
                          ▸
                        </span>
                        {a}
                      </li>
