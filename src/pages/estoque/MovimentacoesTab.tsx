...
type Mov = {
  id: string; tipo: 'Entrada' | 'Saída' | 'Ajuste'; ajusteTipo?: 'entrada' | 'saida';
  dataISO: string; dataDisplay: string; sku: string; produto: string;
  quantidade: number; valor: number; ref: string; origem: string;
  imutavel?: boolean; estoqueAntes?: number; estoqueDepois?: number;
};
...
export function MovimentacoesTab() {
  ...
  const movimentacoes = useMemo((): Mov[] => {
    const entradas: Mov[] = compras.map((c) => ({
      id: `e-${c.id}`, tipo: 'Entrada', dataISO: c.data, dataDisplay: c.data,
      sku: c.sku, produto: c.produto, quantidade: c.quantidadeEntrada, valor: c.custoTotal,
      ref: c.nfRef || '—', origem: c.fornecedor || '—',
    }));
    const saidas: Mov[] = pedidos
      .filter((p) => p.status === 'Concluído' || p.status === 'Enviado')
      .map((p) => ({
        id: `s-${p.id}`, tipo: 'Saída', dataISO: p.data, dataDisplay: p.data,
        sku: p.sku, produto: p.produto, quantidade: p.unidadesEstoque, valor: p.receita,
        ref: p.numeroPedido, origem: p.loja,
      }));
    const ajustesMov: Mov[] = ajustes.map((a) => {
      const dt = new Date(a.criadoEm);
      return {
        id: `a-${a.id}`, tipo: 'Ajuste', ajusteTipo: a.tipo,
        dataISO: a.criadoEm,
        dataDisplay: `${dt.toLocaleDateString('pt-BR')} ${dt.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}`,
        sku: a.sku, produto: a.produto, quantidade: a.quantidade, valor: 0,
        ref: '—', origem: a.motivo || '—',
        imutavel: true, estoqueAntes: a.estoqueAntes, estoqueDepois: a.estoqueDepois,
      };
    });
    return [...entradas, ...saidas, ...ajustesMov].sort((a, b) => b.dataISO.localeCompare(a.dataISO));
  ...
  const movFiltered = useMemo(() => {
    ...
    return movimentacoes.filter((m) => {
      ...
      if (q && !m.sku.toLowerCase().includes(q) && !m.produto.toLowerCase().includes(q) && !m.ref.toLowerCase().includes(q) && !m.origem.toLowerCase().includes(q)) return false;
      return true;
    });
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        <div className="relative flex-1 min-w-48">
          ...
          <input className="input pl-8" placeholder="Buscar SKU, produto, referência ou motivo…" value={movSearch}
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto overflow-y-auto max-h-[calc(100vh-440px)]">
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10 bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700">
              <tr>
                {['Data / Hora', 'Tipo', 'SKU', 'Produto', 'Qtd.', 'Estoque', 'Referência / Motivo'].map((h) => (
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
              {movPaginados.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-12 text-slate-400 text-sm">Nenhuma movimentação encontrada.</td></tr>
              ) : movPaginados.map((m) => {
                ...
                return (
                  <tr key={m.id} className={`hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors ${isAjuste ? 'bg-violet-50/30 dark:bg-violet-900/10' : ''}`}>
                    ...
                    <td className="px-3 py-2.5 font-mono text-xs font-medium text-slate-700 dark:text-slate-300">{m.sku}</td>
                    <td className="px-3 py-2.5 text-slate-800 dark:text-slate-200 whitespace-nowrap max-w-[140px] truncate">{m.produto}</td>
                    <td className={`px-3 py-2.5 font-medium ${isEntrada ? 'text-emerald-600' : 'text-red-500'}`}>
                    ...
                    <td className="px-3 py-2.5 text-xs text-slate-500 dark:text-slate-400">
                      {isAjuste && m.estoqueAntes !== undefined
                        ? <span className="flex items-center gap-1">
                            ...
                            <span className="font-medium text-slate-700 dark:text-slate-200">{m.estoqueDepois}</span>
                          </span>
                        : !isAjuste && m.valor > 0 ? fmt(m.valor) : '—'
                      }
                    </td>
                    <td className="px-3 py-2.5 text-slate-500 dark:text-slate-400 text-xs max-w-[180px] truncate">
                      {isAjuste
                        ? m.origem !== '—' ? <span className="italic">{m.origem}</span> : <span className="text-slate-300 dark:text-slate-600">Sem motivo</span>
        ...
        <PaginationBar page={movPage} total={movFiltered.length} pageSize={movPageSize}
          onPage={setMovPage} onPageSize={setMovPageSize} />
      </div>
    </div>
  );
}
