        ...
        <div className="flex items-center gap-1">
          ...
          {getPages(page, totalPages).map((pg, i) =>
            pg === '…'
              ...
              : <button key={pg} onClick={() => onPage(pg as number)}
                  className={`min-w-[28px] h-7 rounded-lg text-xs font-medium transition-colors ${page === pg ? 'bg-shopee-500 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700'}`}>
                  {pg}
                </button>
          )}
