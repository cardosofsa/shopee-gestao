...
import { Search, X, ChevronUp, ChevronDown, ChevronsUpDown, Pencil, Package } from 'lucide-react';
import {
...
const STATUS_STYLE: Record<StatusEstoque, { badge: string; dot: string }> = {
  ...
  'Estoque Estável':{ badge: 'bg-emerald-50 text-emerald-700 border-emerald-200', dot: 'bg-emerald-400' },
...
const STATUS_ORDER: Record<StatusEstoque, number> = {
  'Comprar': 0, 'Estoque Baixo': 1, 'Estoque Estável': 2, 'Estoque Acima': 3,
};
...
const ALL_STATUSES: StatusEstoque[] = ['Comprar', 'Estoque Baixo', 'Estoque Estável', 'Estoque Acima'];
...
function SortIcon({ col, sort }: { col: string; sort: SortState }) {
  if (!sort || sort.col !== col) return <ChevronsUpDown size={11} className="text-slate-300 flex-shrink-0" />;
  return sort.dir === 'asc'
    ? <ChevronUp size={11} className="text-shopee-500 flex-shrink-0" />
    : <ChevronDown size={11} className="text-shopee-500 flex-shrink-0" />;
}
...
export function PosicaoTab({
  ...
  const toggleStatus = (s: StatusEstoque) => setPosStatuses((prev) => {
  ...
  const posSorted = useMemo(() => {
    ...
    return [...posFiltered].sort((a, b) => {
      ...
      if (col === 'status')        { va = STATUS_ORDER[va as StatusEstoque]; vb = STATUS_ORDER[vb as StatusEstoque]; }
      if (col === 'diasCobertura') { va = isFinite(va) ? va : 99999; vb = isFinite(vb) ? vb : 99999; }
  ...
  const handlePosSort = (col: string) =>
    setPosSort((prev) => prev?.col === col ? (prev.dir === 'asc' ? { col, dir: 'desc' } : null) : { col, dir: 'asc' });

  const SortTh = ({ col, label, className = '' }: { col: string; label: string; className?: string }) => (
    <th onClick={() => handlePosSort(col)}
      ...
      <span className="flex items-center gap-1">{label}<SortIcon col={col} sort={posSort} /></span>
    </th>
  );
    ...
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="card p-4">
          ...
          <p className="text-slate-900 dark:text-slate-100 font-bold text-xl">{fmt(totalEstoque)}</p>
        </div>
        <div className="card p-4">
        ...
        <div className="card p-4">
          ...
          <p className={`font-bold text-xl ${precisamComprar.length > 0 ? 'text-red-500' : 'text-emerald-600'}`}>
            {precisamComprar.length > 0 ? `${precisamComprar.length} SKU${precisamComprar.length > 1 ? 's' : ''}` : 'Nenhum'}
          </p>
        </div>
        <div className="card p-4">
          <p className="text-slate-500 dark:text-slate-400 text-xs mb-1">Cobertura Média</p>
          <p className="text-slate-900 dark:text-slate-100 font-bold text-xl">{coberturaMedia !== null ? `${coberturaMedia}d` : '—'}</p>
        </div>
      </div>
      ...
      {chartData.length > 0 && (
        <div className="card p-4">
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-3">Composição do Estoque — top {chartData.length} SKUs por valor</p>
          <ResponsiveContainer width="100%" height={chartData.length * 32 + 8}>
            <BarChart data={chartData} layout="vertical" margin={{ top: 0, right: 56, left: 0, bottom: 0 }}>
              ...
              <YAxis dataKey="sku" type="category" width={96} tick={{ fontSize: 11, fontFamily: 'monospace', fill: '#94a3b8' }} />
              ...
              <Bar dataKey="valor" radius={[0, 4, 4, 0]}
                label={{
                  content: (props: any) => {
                    ...
                    return (
                      <text x={x + width + 4} y={y + height / 2} fontSize={11} fill="#94a3b8" dominantBaseline="middle">
                        {pct.toFixed(0)}%
                      </text>
                    );
                  },
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap gap-2 items-center">
          <div className="relative flex-1 min-w-48">
            ...
            <input className="input pl-8" placeholder="Buscar SKU ou produto…" value={posSearch}
            ...
            {ALL_STATUSES.map((s) => (
              <button key={s} onClick={() => toggleStatus(s)}
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border transition-all ${
                  posStatuses.has(s) ? STATUS_STYLE[s].badge : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600'
                }`}>
                <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${STATUS_STYLE[s].dot}`} />
                {s}{posStatuses.has(s) && <X size={10} />}
              </button>
            ))}
            {posStatuses.size > 0 && (
              <button onClick={() => setPosStatuses(new Set())}
        ...
        <div className="card overflow-hidden">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                {posPaginados.length === 0 ? (
                  <tr><td colSpan={12} className="text-center py-12 text-slate-400 text-sm">Nenhum SKU encontrado.</td></tr>
                ) : posPaginados.map((p) => (
                  <tr key={p.sku} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">
                    <td className="px-3 py-3 font-mono text-xs font-medium text-slate-700 dark:text-slate-300">{p.sku}</td>
                    <td className="px-3 py-3 text-slate-800 dark:text-slate-200 font-medium whitespace-nowrap">{p.nome}</td>
                    <td className="px-3 py-3 text-emerald-600 font-medium">{p.entradas}</td>
                    <td className="px-3 py-3 text-red-500">{p.saidas}</td>
                    <td className="px-3 py-3 font-bold text-slate-900 dark:text-slate-100">{p.estoqueAtual}</td>
                    <td className="px-3 py-3 text-slate-600 dark:text-slate-300">{fmt(p.custoUnitario)}</td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200 font-medium">{fmt(p.valorEstoque)}</td>
                    <td className="px-3 py-3 text-slate-500 dark:text-slate-400">{p.vendaDia > 0 ? p.vendaDia.toFixed(2) : '—'}</td>
                    <td className="px-3 py-3"><CoverageBar dias={p.diasCobertura} /></td>
                    <td className="px-3 py-3 text-slate-500 dark:text-slate-400">{p.ptReposicao > 0 ? p.ptReposicao : '—'}</td>
                    <td className="px-3 py-3"><StatusBadge status={p.status} /></td>
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => onEditProduto(p.sku)}
                          className="text-slate-300 hover:text-shopee-500 transition-colors" title="Editar produto">
                          <Pencil size={13} />
                        </button>
                        <button onClick={() => onAjuste()}
                          className="text-slate-300 hover:text-shopee-500 transition-colors" title="Ajustar estoque">
                          <Package size={13} />
                        </button>
                      </div>
                    </td>
          ...
          <PaginationBar page={posPage} total={posFiltered.length} pageSize={posPageSize}
            onPage={setPosPage} onPageSize={setPosPageSize} />
        </div>
      </div>
    </>
  );
