...
import {
  FileUp, CheckCircle2, AlertTriangle, XCircle, Upload,
  Clock, ChevronRight, RotateCcw, Store,
} from 'lucide-react';
        ...
        <tbody>
          {rows.map((p, i) => (
            <tr key={i} className="border-t border-slate-100 dark:border-slate-800">
              <td className="px-3 py-2 whitespace-nowrap text-slate-600 dark:text-slate-300">{p.data}</td>
              <td className="px-3 py-2 whitespace-nowrap font-mono text-slate-700 dark:text-slate-200">{p.numeroPedido}</td>
              <td className="px-3 py-2 whitespace-nowrap text-slate-600 dark:text-slate-300">{p.sku}</td>
              <td className="px-3 py-2 max-w-[180px] truncate text-slate-600 dark:text-slate-300">{p.produto}</td>
              <td className="px-3 py-2 text-center text-slate-600 dark:text-slate-300">{p.quantidade}</td>
              <td className="px-3 py-2 whitespace-nowrap">
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
                  p.status === 'Concluído' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                  p.status === 'Devolvido' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                  p.status === 'Enviado'   ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
...
export default function Importar() {
  ...
  const [lastDone, setLastDone] = useState<{ novos: number; duplicados: number; formato: ImportFormato } | null>(null);
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (parsed.length === 0) { toast('Nenhum pedido válido encontrado. Verifique o arquivo.', 'error'); return; }
      ...
      const lojaDefault  = isShopeeNativo ? (lojasDisponiveis[0] ?? configuracoes.lojas[0] ?? 'Ambas') : '';
  ...
  const handleConfirm = useCallback(() => {
    ...
    const final = isShopeeNativo && lojaCustom
  ...
  const handleReset = () => { setPending(null); setLastDone(null); };
  ...
  return (
    <div className="flex-1 p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Centro de Importação</h1>
        ...
        <div className="flex items-start gap-3 p-4 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-green-800 dark:text-green-300">
              {lastDone.novos} pedido{lastDone.novos !== 1 ? 's' : ''} importado{lastDone.novos !== 1 ? 's' : ''}
            </p>
          ...
          <button onClick={handleReset} className="text-xs text-green-700 dark:text-green-400 hover:underline shrink-0">
            Nova importação
          </button>
        </div>
      )}
      ...
      {pending && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700">
            <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${FORMAT_COLOR[pending.formato]}`}>
            ...
            {pending.duplicados > 0 && (
              <div className="flex items-center gap-1.5 text-sm text-slate-700 dark:text-slate-200">
                ...
                <span className="text-slate-400 dark:text-slate-500">duplicados (serão ignorados)</span>
              </div>
            )}
          </div>
          ...
          {pending.isShopeeNativo && (
            <div className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900">
              ...
              <label className="text-xs text-slate-500 dark:text-slate-400 shrink-0">Atribuir à loja</label>
              <select
          ...
          {pending.novos.some((p) => p.receita === 0) && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-xs text-amber-700 dark:text-amber-400">
              ...
              Alguns pedidos têm receita R$ 0,00. Verifique se o arquivo está completo ou se os SKUs estão cadastrados.
            </div>
          )}
      ...
      {!pending && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {([
            { fmt: 'shopee_nativo' as ImportFormato, title: 'Shopee Nativo', desc: 'Exportado direto do Shopee Seller Center. Colunas: "ID do pedido", "Número de referência SKU".' },
            { fmt: 'upseller'      as ImportFormato, title: 'UpSeller',       desc: 'Exportado do painel UpSeller. Colunas: "Nº de Pedido da Plataforma", "Nome da Loja no UpSeller".' },
            { fmt: 'generico'      as ImportFormato, title: 'Genérico',        desc: 'Formato próprio ou exportações de outras ferramentas. Adapta automaticamente.' },
          ] as const).map(({ fmt: f, title, desc }) => (
            <div key={f} className="p-3 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${FORMAT_COLOR[f]}`}>{title}</span>
      ...
      {history.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Histórico recente</p>
          </div>
          <div className="space-y-1.5">
            {history.map((h) => (
              <div key={h.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-xs">
                <span className={`px-2 py-0.5 rounded-full font-semibold shrink-0 ${FORMAT_COLOR[h.formato]}`}>
                  {FORMAT_LABEL[h.formato]}
                </span>
                <span className="font-medium text-slate-700 dark:text-slate-200">{h.novos} novos</span>
                {h.duplicados > 0 && (
                ...
                <span className="ml-auto text-slate-400 dark:text-slate-500 whitespace-nowrap">{fmtTs(h.ts)}</span>
