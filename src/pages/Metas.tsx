...
function PaceBadge({
  ...
  return (
    <div
      className={`inline-flex items-center gap-1 text-[10px] font-medium px-2.5 py-1 rounded-full mt-2 ${
        noRitmo
...
export default function Metas() {
  ...
  const historicoOrdenado = useMemo(() => {
    ...
    const linhas = hist.map((h) => ({
      ...
      lucro: h.lucroOperacional,
      margem: h.margemPercentual,
      isLive: false,
    }));
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                {[...historicoOrdenado].reverse().map((h) => {
                  ...
                  return (
                    <tr
                      key={h.mes}
                      className={`hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors ${h.isLive ? 'bg-emerald-50/40 dark:bg-emerald-950/10' : ''}`}
                    >
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          {h.isLive && (
                            <span className="text-[9px] bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 px-1.5 py-0.5 rounded-full font-bold tracking-wide">
                              AO VIVO
                            </span>
                          )}
                        </div>
                      ...
                      {metaFat && (
                        <td className={`px-4 py-2.5 font-bold text-xs ${pctColor(pF)}`}>
                          {pF.toFixed(0)}%
                        </td>
                      )}
                      ...
                      {metaPedidos && (
                        <td className={`px-4 py-2.5 font-bold text-xs ${pctColor(pP)}`}>
                          {pP.toFixed(0)}%
                        </td>
                      )}
                      <td
                        className={`px-4 py-2.5 font-medium ${h.lucro >= 0 ? 'text-emerald-600' : 'text-red-500'}`}
                      >
                        {fmt(h.lucro)}
                      </td>
                      {metaLucro && (
                        <td className={`px-4 py-2.5 font-bold text-xs ${pctColor(pL)}`}>
                          {pL.toFixed(0)}%
                        </td>
                      )}
                      <td
                        className={`px-4 py-2.5 font-medium ${h.margem >= 20 ? 'text-emerald-600' : h.margem >= 10 ? 'text-amber-500' : 'text-red-500'}`}
                      >
                        {fmtPct(h.margem)}
                      </td>
                      {metaMargem && (
                        <td className={`px-4 py-2.5 font-bold text-xs ${pctColor(pM)}`}>
                          {pM.toFixed(0)}%
                        </td>
                      )}
                      <td className="px-4 py-2.5">
                        <ResultBadge pct={resultado} semMeta={!temMeta} />
                      </td>
                    </tr>
                  );
                })}
      ...
      {!temHistorico && !temMeta && (
        <div className="card p-12 text-center">
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm max-w-sm mx-auto">
            ...
            em cada card acima para começar.
          </p>
        </div>
      )}
    </div>
