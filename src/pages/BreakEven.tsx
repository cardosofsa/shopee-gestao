...
function Gauge({ pct, label, cor }: { pct: number; label: string; cor: string }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <p
        className={`text-xs font-semibold mt-1 ${pct >= 100 ? 'text-emerald-600' : pct >= 75 ? 'text-amber-500' : 'text-slate-500'}`}
      >
        {label}
      </p>
    </div>
...
function KCard({
  ...
  return (
    <div className="card p-4">
      ...
      <p className={`text-xl font-bold ${accent ?? 'text-slate-900 dark:text-slate-100'}`}>
        {value}
      </p>
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-700 last:border-0">
      ...
      {editing ? (
        ...
        <button
          onClick={() => {
            setDraft(String(value > 0 ? value.toFixed(2) : ''));
            setEditing(true);
          }}
        ...
        >
          {value > 0 ? (
            ...
            <span className="text-slate-300 dark:text-slate-600 text-xs">+ definir</span>
          )}
        </button>
      )}
    </div>
...
export default function BreakEven() {
  ...
  const pedidos = useMemo(
    () =>
      (lojaFiltro ? pedidosAll.filter((p) => p.loja === lojaFiltro) : pedidosAll).filter(
        (p) => p.data.startsWith(mes) && (p.status === 'Concluído' || p.status === 'Enviado')
      ),
  ...
  const bePedidos =
    beReceita !== null && ticketMedio > 0 ? Math.ceil(beReceita / ticketMedio) : null;
  ...
  const diasParaBE =
    beReceita !== null && receitaDia > 0 && receita < beReceita
    ...
    <div className="p-4 md:p-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              Quanto preciso vender para cobrir todos os custos
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <div className="card p-6 flex flex-col items-center justify-center text-center">
          ...
          <div className="mt-4 space-y-1">
            <p className="text-sm text-slate-600 dark:text-slate-300">
              ...
              <span className="font-bold">{beReceita != null ? fmt(beReceita) : '—'}</span>{' '}
              necessários
            </p>
        ...
        <div className="card p-5">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 mb-4">
            Valores auto-calculados do mês · ajustáveis manualmente
          </p>
          <div>
            <EditableRow
              label="Despesas operacionais"
              value={despCfg}
              onChange={(v) => setOverride('despesas', v)}
            />
            <EditableRow
              label="Contas a pagar do mês"
              value={contasCfg}
              onChange={(v) => setOverride('contas', v)}
            />
            <EditableRow
              label="DAS / Imposto estimado"
              value={dasCfg}
              onChange={(v) => setOverride('das', v)}
            />
            <EditableRow
              label="Outros custos fixos"
              value={outroCfg}
              onChange={(v) => setOverride('outro', v)}
            />
          ...
          <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            <span className="flex items-center gap-1">
              <Info size={11} /> Margem de Contribuição
            </span>
            <span
              className={`font-bold ${mcr >= 0.2 ? 'text-emerald-600' : mcr >= 0.1 ? 'text-amber-500' : 'text-red-500'}`}
            >
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <KCard
          ...
          icon={DollarSign}
          accent={
            beReceita != null && receita >= beReceita
              ? 'text-emerald-600'
        ...
        <KCard
          ...
          icon={ShoppingCart}
          accent={
            bePedidos != null && pedidos.length >= bePedidos
              ? 'text-emerald-600'
              : 'text-slate-700 dark:text-slate-200'
          }
        />
            ...
            <AreaChart data={dailyData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <Tooltip
                formatter={(v: unknown, name: unknown) => [
                  fmt(Number(v)),
                  name === 'acumulado' ? 'Receita Acum.' : 'PE',
                ]}
              ...
              <ReferenceLine
                ...
                stroke={C.red}
                strokeDasharray="4 4"
                label={{ value: 'Equilíbrio', position: 'right', fontSize: 10, fill: C.red }}
              />
              <Area
                type="monotone"
                dataKey="acumulado"
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={historico6m} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <Tooltip
                formatter={(v: unknown, name: unknown) => [
                  fmt(Number(v)),
                  name === 'receita' ? 'Receita' : 'Break-Even',
                ]}
                contentStyle={{ fontSize: 12 }}
              />
              <Area
          ...
          <div className="flex items-center gap-4 mt-2 text-[10px] text-slate-400 justify-end">
            <span className="flex items-center gap-1.5">
              <span className="w-4 h-0.5 bg-core-green inline-block rounded" /> Receita
            </span>
            <span className="flex items-center gap-1.5">
              <span
                className="w-4 h-0.5 bg-red-400 inline-block rounded"
                style={{ borderTop: '2px dashed #f87171', background: 'transparent' }}
              />{' '}
              Break-Even
            </span>
          </div>
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs text-slate-400 dark:text-slate-500">
              — impacto da margem de contribuição no PE
            </span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700">
                  {['Cenário', 'MC Ajustada', 'PE em Receita', 'PE em Pedidos', 'Vs atual'].map(
                    (h) => (
                      <th
                        key={h}
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                {sensibilidade.map(({ delta, mcr: mcrAdj, be }) => {
                  ...
                  return (
                    <tr
                      key={delta}
                      className={`transition-colors ${isBase ? 'bg-core-green/5 dark:bg-core-green/5' : 'hover:bg-slate-50 dark:hover:bg-slate-700/20'}`}
                    >
      ...
      {pedidos.length === 0 && (
        <div className="card p-12 text-center">
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm font-medium">
            Sem pedidos neste mês para calcular o equilíbrio.
          </p>
          <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">
            Você ainda pode ajustar os custos fixos para estimar o ponto de equilíbrio.
          </p>
        </div>
      )}
    </div>
