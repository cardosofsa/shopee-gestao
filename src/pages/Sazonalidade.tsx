...
import {
  CalendarRange, TrendingUp, Award, Zap,
} from 'lucide-react';
import {
  ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid,
  Tooltip, Cell, RadarChart, Radar, PolarGrid, PolarAngleAxis, PolarRadiusAxis,
} from 'recharts';
...
function HeatmapCell({ val, max, date, orders }: { val: number; max: number; date: string; orders: number }) {
  ...
  return (
    ...
    >
      <div className={`w-[13px] h-[13px] rounded-[2px] cursor-pointer transition-opacity hover:opacity-75 ${heatColor(intensity)}`} />
      {tip && val > 0 && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 z-50 pointer-events-none
          ...
          <p className="text-slate-400">{orders} pedido{orders !== 1 ? 's' : ''}</p>
        </div>
      )}
    </div>
  );
...
function Stat({ label, value, sub, icon: Icon, color = 'text-core-green' }: {
...
export default function Sazonalidade() {
  ...
  const melhorDia = useMemo(() => diasData.reduce((b, d) => (d.media > b.media ? d : b), diasData[0]), [diasData]);
  const piorDia   = useMemo(() => diasData.filter((d) => d.pedidos > 0).reduce((b, d) => (d.media < b.media ? d : b), diasData[0]), [diasData]);
  ...
  const melhorMes = useMemo(() => mesesData.reduce((b, m) => (m.media > b.media ? m : b), mesesData[0]), [mesesData]);

  // ── Fim de semana vs semana ───────────────────────────────────────────────
  const fdsVsWeek = useMemo(() => {
    let fdsRec = 0, fdsN = 0, weekRec = 0, weekN = 0;
    pedidos.forEach((p) => {
      const dow = parseDate(p.data).getDay();
      if (dow === 0 || dow === 6) { fdsRec += p.receita; fdsN++; }
      else { weekRec += p.receita; weekN++; }
    });
    ...
    return (
      <div className="p-6">
        <div className="card p-14 text-center">
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm">Importe pedidos para ver a análise de sazonalidade.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">Padrões de vendas por dia, semana e mês</p>
          </div>
        </div>
        {/* Métrica toggle */}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <Stat label="Melhor dia" value={melhorDia.label} sub={`média ${fmt(melhorDia.media)}`} icon={Award} />
        <Stat label="Melhor mês" value={melhorMes.label} sub={`média ${fmt(melhorMes.media)}/ano`} icon={TrendingUp} />
        <Stat
          label="Fim de semana"
      ...
      <div className="card p-5">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3">
          Heatmap — {metric === 'receita' ? 'Receita' : 'Pedidos'} por dia (últimas {SEMANAS} semanas)
        </p>
        ...
        <div className="relative" style={{ paddingLeft: '28px' }}>
          <div className="flex gap-[3px] mb-1 text-[9px] text-slate-400 dark:text-slate-500">
            {heatMonthLabels.map(({ week, label }) => (
              <div key={week} style={{ position: 'absolute', left: `${28 + week * 16}px` }}>{label}</div>
            ))}
          </div>
          <div className="mt-3 flex gap-[3px]">
            ...
            <div className="flex flex-col gap-[3px] mr-1">
              {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((d, i) => (
                <div key={i} className="w-[13px] h-[13px] text-[8px] text-slate-400 flex items-center justify-center">
                  {i % 2 === 1 ? d : ''}
                </div>
              ))}
            </div>
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={diasData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 10, fill: C.slate }} tickFormatter={(v) => `R$${(v / 1000).toFixed(1)}k`} axisLine={false} tickLine={false} width={44} />
              <Tooltip
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <RadarChart data={radarData}>
              ...
              <Radar dataKey="value" stroke={C.primary} fill={C.primary} fillOpacity={0.18} strokeWidth={2} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="card p-5">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={mesesData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="label" tick={{ fontSize: 11, fill: C.slate }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 10, fill: C.slate }} tickFormatter={(v) => `R$${(v / 1000).toFixed(1)}k`} axisLine={false} tickLine={false} width={44} />
            <Tooltip formatter={(v: any) => [fmt(Number(v)), 'Média anual']} contentStyle={{ fontSize: 12 }} />
            <Bar dataKey="media" radius={[4, 4, 0, 0]}>
              {mesesData.map((m, i) => (
                <Cell
                  key={i}
                  fill={m.label === melhorMes.label ? C.primary : m.media > melhorMes.media * 0.7 ? '#34d399' : C.slate}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                {topPorMes.map((m) => (
                  <tr key={m.label} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">
                    <td className="px-4 py-2.5 font-medium text-slate-700 dark:text-slate-200 w-20">{m.label}</td>
                    <td className="px-4 py-2.5 text-slate-600 dark:text-slate-300 truncate max-w-xs">{m.produto}</td>
                    <td className="px-4 py-2.5 font-mono text-core-green font-medium">{fmt(m.receita)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="card p-4 border-l-4 border-l-core-green">
          <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 mb-1">Melhor dia da semana</p>
          <p className="text-2xl font-bold text-core-green">{melhorDia.label}</p>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Média de {fmt(melhorDia.media)} · {melhorDia.pedidos} pedido{melhorDia.pedidos !== 1 ? 's' : ''} total
          </p>
        </div>
        <div className="card p-4 border-l-4 border-l-amber-400">
          <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 mb-1">Melhor mês histórico</p>
          <p className="text-2xl font-bold text-amber-500">{melhorMes.label}</p>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Média de {fmt(melhorMes.media)} por ano · {melhorMes.pedidos} pedido{melhorMes.pedidos !== 1 ? 's' : ''}
          </p>
        </div>
        <div className="card p-4 border-l-4 border-l-slate-300">
          <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 mb-1">Fim de semana vs Semana</p>
          <p className={`text-2xl font-bold ${fdsVsWeek.fdsAvg >= fdsVsWeek.weekAvg ? 'text-core-green' : 'text-slate-500'}`}>
