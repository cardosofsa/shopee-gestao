...
import {
  ...
  Truck, Download, ChevronDown, ChevronUp, ShoppingBag, Package,
} from 'lucide-react';
...
const URG_CFG: Record<Urgencia, { label: string; dot: string; row: string; badge: string; icon: React.ElementType }> = {
  critico: {
    label: 'Crítico',
...
function KCard({ label, value, sub, color, icon: Icon }: {
  ...
  return (
    <div className="card p-4">
      ...
      <p className={`text-xl font-bold ${color ?? 'text-slate-900 dark:text-slate-100'}`}>{value}</p>
...
function GrupoFornecedor({ fornecedor, itens }: {
  fornecedor: string;
  itens: ItemReposicao[];
}) {
    ...
    <div className="card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-semibold text-sm text-slate-800 dark:text-slate-100">{fornecedor}</span>
          {temCritico && <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />}
          <span className="text-xs text-slate-500 dark:text-slate-400">· {itens.length} SKU{itens.length > 1 ? 's' : ''}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-sm font-bold text-slate-700 dark:text-slate-200 font-mono">{fmt(totalCusto)}</span>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100 dark:border-slate-700">
              {['Status', 'Produto', 'Estoque', 'Cobertura', 'Ponto Rep.', 'Qtd sugerida', 'Custo unit.', 'Custo total'].map((h) => (
          ...
          <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
            {itens.map((item) => {
              ...
              return (
                <tr key={item.sku} className={`hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors ${cfg.row}`}>
                  <td className="px-3 py-2.5">
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 w-fit ${cfg.badge}`}>
                      <span className={`w-1 h-1 rounded-full ${cfg.dot}`} />
                      {cfg.label}
                    </span>
                  </td>
                  <td className="px-3 py-2.5">
                    <p className="font-medium text-slate-800 dark:text-slate-100 truncate max-w-[160px]">{item.nome}</p>
                    <p className="text-xs text-slate-400 font-mono">{item.sku}</p>
                  </td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-1">
                      <span className={`font-bold text-sm ${item.estoqueAtual <= item.estoqueSeguranca ? 'text-red-500' : 'text-slate-700 dark:text-slate-200'}`}>
                        {item.estoqueAtual}
                      </span>
                      {item.estoqueSeguranca > 0 && (
                        <span className="text-[9px] text-slate-400">/ min {item.estoqueSeguranca}</span>
                      )}
                    </div>
                  </td>
                  <td className="px-3 py-2.5">
                    <span className={`text-sm font-medium ${
                      item.diasCobertura < 7 ? 'text-red-500' :
                      item.diasCobertura < 30 ? 'text-amber-500' : 'text-emerald-600'
                    }`}>
                      {item.vendaDia > 0
                        ? item.diasCobertura === Infinity ? '∞' : `${Math.round(item.diasCobertura)}d`
                        : '—'}
                    </span>
                  </td>
                  ...
                  <td className="px-3 py-2.5">
                    <span className={`font-bold text-sm ${item.qtdSugerida > 0 ? 'text-core-green' : 'text-slate-400'}`}>
          ...
          <tfoot>
            <tr className="border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
              <td colSpan={6} />
              <td className="px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-slate-500">Total pedido</td>
...
export default function Reposicao() {
  ...
  const fornecedorPorSku = useMemo(() => {
    ...
    compras
      ...
      .forEach((c) => { if (c.fornecedor) map.set(c.sku, c.fornecedor); });
    return map;
  }, [compras]);

  // Build items
  const items: ItemReposicao[] = useMemo(() => {
    const cutoff = new Date(hoje); cutoff.setDate(hoje.getDate() - JANELA_VEND);
  ...
  const itemsFiltrados = useMemo(() => {
    return items
      .filter((i) => {
        ...
        if (!showOK && filtro === 'todos' && i.urgencia === 'ok' && i.qtdSugerida === 0) return false;
        if (busca) {
  ...
  const grupos = useMemo(() => {
    ...
    return [...map.entries()]
      .sort(([, a], [, b]) => {
        ...
        return b.reduce((s, i) => s + i.custoEstimado, 0) - a.reduce((s, i) => s + i.custoEstimado, 0);
      });
  }, [itemsFiltrados]);
  ...
  const exportar = () => {
    const headers = ['SKU', 'Produto', 'Fornecedor', 'Estoque Atual', 'Estoque Mín.', 'Venda/Dia (30d)', 'Cobertura (dias)', 'Qtd Sugerida', 'Custo Unit. (R$)', 'Custo Total (R$)', 'Urgência'];
    const rows = itemsFiltrados.map((i) => [
      i.sku, i.nome, i.fornecedor, i.estoqueAtual, i.estoqueSeguranca,
      i.vendaDia.toFixed(2), i.diasCobertura === Infinity ? '∞' : Math.round(i.diasCobertura),
      i.qtdSugerida, i.custoUnitario.toFixed(2), i.custoEstimado.toFixed(2), URG_CFG[i.urgencia].label,
    ]);
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">Reposição de Estoque</h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              Velocidade dos últimos {JANELA_VEND} dias · cobertura alvo {COBERTURA} dias · lead time {LEAD_TIME} dias
            </p>
          </div>
        </div>
      ...
      {criticos > 0 && (
        <div className="flex items-start gap-3 p-4 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-xl">
          ...
          <p className="text-sm text-red-700 dark:text-red-400">
            <span className="font-semibold">{criticos} produto{criticos > 1 ? 's' : ''} em estado crítico</span> — estoque abaixo do mínimo ou cobertura inferior a {LEAD_TIME} dias. Pedido urgente necessário.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <KCard label="Críticos"        value={String(criticos)} sub="abaixo do mínimo ou < 7d"     color={criticos > 0 ? 'text-red-500' : 'text-slate-400'}    icon={AlertTriangle} />
        <KCard label="Atenção"         value={String(atencao)}  sub="cobertura entre 7 e 30 dias"  color={atencao > 0 ? 'text-amber-500' : 'text-slate-400'}   icon={Clock} />
        <KCard label="Custo do pedido" value={fmt(custoTotal)}  sub={`${itemsFiltrados.filter(i=>i.qtdSugerida>0).length} SKUs a repor`} color="text-slate-900 dark:text-slate-100" icon={ShoppingBag} />
        <KCard label="Fornecedores"    value={String(nFornecedores)} sub="agrupados na lista"       color="text-slate-900 dark:text-slate-100"                  icon={Truck} />
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex gap-1.5">
          {([
            ...
            ['atencao', 'Atenção'],
            ['ok',      'OK'],
          ] as [FiltroUrgencia, string][]).map(([f, label]) => (
            <button
              key={f}
              onClick={() => setFiltro(f)}
              className={`text-xs px-3 py-1.5 rounded-full font-medium transition-colors whitespace-nowrap ${
                filtro === f ? 'bg-core-green text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'
              }`}
            >
              {label}
            </button>
      ...
      {grupos.length === 0 ? (
        <div className="card p-14 text-center">
          ...
          <h3 className="font-semibold text-slate-700 dark:text-slate-200 mb-1">Nenhum produto encontrado</h3>
          ...
          {custoTotal > 0 && (
            <div className="flex items-center justify-between px-4 py-3 bg-core-green/5 border border-core-green/20 rounded-xl">
              <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                Investimento total estimado — {itemsFiltrados.filter(i => i.qtdSugerida > 0).length} SKUs de {nFornecedores} fornecedor{nFornecedores > 1 ? 'es' : ''}
              </span>
