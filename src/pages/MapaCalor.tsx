...
import {
  Activity, ChevronLeft, ChevronRight, TrendingUp, Award,
  BarChart2, Calendar,
} from 'lucide-react';
import {
...
const RAMP = ['#f1f5f9', '#bbf7d0', '#86efac', '#4ade80', '#22c55e', '#16a34a', '#15803d', '#166534'];
...
function endOfYear(year: number)   { return new Date(year, 11, 31); }
...
function monthPositions(_year: number, weeks: (string | null)[][]): { label: string; col: number }[] {
  ...
  return (
    <div className="bg-slate-800 text-white text-[11px] rounded-lg px-3 py-2 shadow-xl pointer-events-none">
      ...
      <p className="text-slate-300">
        {METRIC_CFG[metric].label}: <span className="text-white font-bold">{METRIC_CFG[metric].fmt(value)}</span>
      </p>
    </div>
  );
}
...
export default function MapaCalor() {
  ...
  const [hovered, setHovered] = useState<{ day: string; value: number; x: number; y: number } | null>(null);
  ...
  const dayMap = useMemo<Map<string, { receita: number; pedidos: number; lucro: number }>>(() => {
    const m = new Map();
    pedidosAll
      .filter((p) => (p.status === 'Concluído' || p.status === 'Enviado') && p.data.startsWith(String(year)))
      .forEach((p) => {
    ...
    <div className="flex-1 p-6 space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Mapa de Calor de Vendas</h1>
            <p className="text-xs text-slate-400 dark:text-slate-500">Padrões diários de venda ao longo do ano</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-1 card px-2 py-1">
          ...
          <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 px-1">{year}</span>
          <button
            onClick={() => setYear((y) => y + 1)}
        ...
        <div className="flex items-center gap-1 card px-1 py-1">
          {(Object.entries(METRIC_CFG) as [Metric, typeof METRIC_CFG[Metric]][]).map(([k, cfg]) => (
            <button
              key={k}
              onClick={() => setMetric(k)}
            ...
            {weeks.map((week, wi) => (
              <div key={wi} className="flex flex-col" style={{ gap: GAP }}>
                {week.map((day, di) => {
                  ...
                  const color = day && val > 0 ? rampColor(val, maxVal) : day ? '#e2e8f0' : 'transparent';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="card p-4">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={monthlyData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 9, fill: C.slate }} width={metric === 'pedidos' ? 28 : 50}
                tickFormatter={metric === 'pedidos' ? (v) => String(v) : (v) => `${(v / 1000).toFixed(0)}k`}
              />
              <Tooltip
                contentStyle={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '12px', fontSize: '11px' }}
                formatter={(v: unknown) => [METRIC_CFG[metric].fmt(v as number), METRIC_CFG[metric].label]}
              />
              <Bar dataKey="total" radius={[4, 4, 0, 0]}>
                {monthlyData.map((m, i) => (
                  <Cell key={i} fill={m.total === Math.max(...monthlyData.map((x) => x.total)) ? C.primary : '#cbd5e1'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Por dia da semana</p>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={dowData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <Tooltip
                contentStyle={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '12px', fontSize: '11px' }}
                formatter={(v: unknown) => [METRIC_CFG[metric].fmt(v as number), `Média por ${METRIC_CFG[metric].label.toLowerCase()}`]}
                ...
                return (
                  <div key={day} className="flex items-center gap-3">
                    <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${
                      i === 0 ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400'
                    }`}>{i + 1}</span>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="h-1 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                        <div className="h-full bg-core-green rounded-full" style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="card p-4">
          ...
          <div className="space-y-3">
              ...
              <div className="flex gap-3 p-3 rounded-xl bg-core-green/5 border border-core-green/10">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                    Média de {METRIC_CFG[metric].fmt(bestDow.media)} por {bestDow.label.toLowerCase()}. Programe campanhas relâmpago neste dia.
                  </p>
                </div>
              </div>
            )}
            ...
            {(() => {
              ...
              const worstMonth = monthlyData.filter((m) => m.total > 0).reduce((b, m) => (m.total < b.total ? m : b), monthlyData.find((m) => m.total > 0) ?? monthlyData[0]);
              ...
              return (
                <div className="flex gap-3 p-3 rounded-xl bg-cyan-50 dark:bg-cyan-950/20 border border-cyan-100 dark:border-cyan-900/30">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                      {METRIC_CFG[metric].fmt(bestMonth.total)} — {worstMonth && worstMonth.total > 0 && worstMonth.label !== bestMonth.label ? `${worstMonth.label} foi o mais fraco com ${METRIC_CFG[metric].fmt(worstMonth.total)}` : 'planeje reposição antes desse período'}.
                    </p>
                  </div>
                </div>
              );
            ...
            {diasComVenda > 0 && diasComVenda < 180 && (
              <div className="flex gap-3 p-3 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30">
                ...
                <div>
                  <p className="text-xs font-semibold text-slate-700 dark:text-slate-200">Consistência a melhorar</p>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                    Vendas em {diasComVenda} dos {year === currentYear ? Math.ceil((Date.now() - new Date(year, 0, 1).getTime()) / 86_400_000) : 365} dias. Ative ADS em dias sem venda para regularizar o fluxo.
                  </p>
                </div>
              </div>
            )}
