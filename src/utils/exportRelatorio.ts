...
import type { Despesa, Pedido, Produto } from '../types';
...
export async function exportarRelatorioMensal(
  ...
  const dreRows = [
    ...
    [
      ...
      moeda(dre.despesasOperacionais),
      pct(f > 0 ? (dre.despesasOperacionais / f) * 100 : 0),
    ],
    ['', '', ''],
    ['RESULTADOS', '', ''],
    ...
    [
      'Lucro Operacional',
      moeda(dre.lucroOperacional),
      pct(f > 0 ? (dre.lucroOperacional / f) * 100 : 0),
    ],
  ...
  const rankingRows = ranking.map((r, i) => [
    ...
    moeda(r.receita),
    moeda(r.ticketMedio),
    moeda(r.lucroOperacional),
    pct(r.margem),
    pct(r.percentReceita),
    r.curvaABC,
  ]);
  ...
  const pedidosRows = pedidos
    ...
    .map((p) => [
      ...
      moeda(p.custoTotal),
      moeda(p.taxaShopee),
      moeda(p.adsMarketing),
      moeda(p.lucroOperacional),
      pct(p.margemSCustoTotal),
    ]);
  ...
  const estoqueRows = produtos.map((p) => {
    ...
    return [
      ...
      moeda(p.estoqueAtual * p.custoUnitario),
      vendaDia.toFixed(2),
    ];
  });
