...
export type InsightCategoria =
  | 'receita' | 'margem' | 'estoque' | 'ads' | 'produtos' | 'metas' | 'giro';
...
const TIPO_PRIORITY: Record<InsightTipo, number> = {
  critico: 0, atencao: 1, positivo: 2,
};
...
export function generateInsights({
  ...
  if (h0 && h1 && h1.faturamentoBruto > 0) {
    ...
    if (delta >= 5) {
      insights.push({
        id: id('r-up'),
        tipo: 'positivo', categoria: 'receita',
      ...
      insights.push({
        id: id('r-down'),
        tipo: 'critico', categoria: 'receita',
  ...
  if (h0 && h1 && h1.ticketMedio > 0) {
    ...
    if (dTicket >= 5) {
      ...
      insights.push({
        id: id('t-down'),
        tipo: 'atencao', categoria: 'receita',
  ...
  if (h0 && h1) {
    ...
    if (dMargem <= -3) {
      ...
      insights.push({
        id: id('m-up'),
        tipo: 'positivo', categoria: 'margem',
  ...
  if (h0 && h1) {
    ...
    if (acosH0 > 12 && acosH1 > 12) {
      insights.push({
        ...
        diagnostico: 'Revise a segmentação das campanhas Shopee Ads e pause anúncios com retorno negativo.',
      });
    } else if (acosH0 > 12) {
      insights.push({
        id: id('ads-1m'),
        tipo: 'atencao', categoria: 'ads',
  ...
  if (rupturas.length > 0) {
    ...
    insights.push({
      id: id('e-ruptura'),
      tipo: 'critico', categoria: 'estoque',
      titulo: `Ruptura em menos de 7 dias`,
      descricao: rupturas.length === 1
      ...
      diagnostico: 'Emita pedido de compra urgente ou registre uma entrada manual em Estoque → Ajustes.',
    });
  }
  ...
  if (zerados.length > 0) {
    insights.push({
      ...
      descricao: `${zerados.slice(0, 3).map(p => `"${p.nome.slice(0, 22)}"`).join(', ')}${zerados.length > 3 ? ` e mais ${zerados.length - 3}` : ''} com estoque zerado.`,
      valor: `${zerados.length} zerado${zerados.length > 1 ? 's' : ''}`,
    });
  }

  // Abaixo do mínimo (mas não zerado)
  const abaixoMin = produtosAtivos.filter(p =>
    p.estoqueSeguranca > 0 && p.estoqueAtual > 0 && p.estoqueAtual < p.estoqueSeguranca,
  );
  if (abaixoMin.length > 0) {
    insights.push({
      id: id('e-baixo'),
      tipo: 'atencao', categoria: 'estoque',
      titulo: `${abaixoMin.length} produto${abaixoMin.length > 1 ? 's' : ''} abaixo do mínimo`,
      descricao: `${abaixoMin.slice(0, 3).map(p => `"${p.nome.slice(0, 22)}"`).join(', ')}${abaixoMin.length > 3 ? ` e mais ${abaixoMin.length - 3}` : ''} abaixo do ponto de segurança.`,
  ...
  if (comSeguranca.length > 0 && abaixoMin.length === 0 && zerados.length === 0) {
    insights.push({
      id: id('e-ok'),
      tipo: 'positivo', categoria: 'estoque',
  ...
  const parados = produtosAtivos.filter(p =>
    p.estoqueAtual > 0 && skusComVenda.has(p.sku) && !skusRecentes.has(p.sku),
  );
  if (parados.length > 0) {
    insights.push({
      id: id('p-parado'),
      tipo: 'atencao', categoria: 'produtos',
      titulo: `${parados.length} produto${parados.length > 1 ? 's' : ''} parado${parados.length > 1 ? 's' : ''}`,
      descricao: `${parados.slice(0, 3).map(p => `"${p.nome.slice(0, 22)}"`).join(', ')}${parados.length > 3 ? ` e mais ${parados.length - 3}` : ''} sem venda nos últimos 30 dias, mas com estoque disponível.`,
  ...
  if (rankingUlt30.length >= 5 && rankingUlt30[0].percentReceita < 25) {
    insights.push({
      ...
      tipo: 'positivo', categoria: 'produtos',
  ...
  if (configuracoes.metaFaturamento && h0) {
    ...
    const mesNome = new Date(h0.mesAno + '-02').toLocaleString('pt-BR', { month: 'long', year: 'numeric' });
    if (prog >= 100) {
      insights.push({
        id: id('g-ok'),
        tipo: 'positivo', categoria: 'metas',
