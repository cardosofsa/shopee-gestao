...
export function mapearSKU(variantSku: string, principalSku: string): { sku: string; kit: number } {
  const s = variantSku.toUpperCase(), p = principalSku.toUpperCase();
  ...
  if (p.includes('FITAANTIFURO-BIKE') || p.includes('FITAANTIFURO_BIKE')) return { sku: 'FITA-BIKE', kit };
  if (p.includes('FITAANTIFURO-MOTO') || p.includes('FITAANTIFURO_MOTO')) return { sku: 'FITA-MOTO', kit };
  if (p.includes('FITAANTIFURO-PCX')  || p.includes('FITAANTIFURO_PCX'))  return { sku: 'FITA-PCX',  kit };
