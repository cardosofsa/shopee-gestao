...
import {
  ...
  Percent, Layers, AlertTriangle, PieChart, FlaskConical,
  HeartPulse, Activity, Bookmark, CalendarSearch, Telescope, CalendarRange,
} from 'lucide-react';
import { useStore } from '../../../store';
import { fmt, fmtNum, fmtPct, getKPIsMes, agruparPorDia, getRankingProdutos, getStatusEstoque, getMesAnterior, getCapitalEstoque, getProjecaoMensal } from '../../../utils/calculations';
import { useAlertas } from '../../../hooks/useAlertas';
import Onboarding from '../../../components/Onboarding';
import InsightsBanner from '../../../components/InsightsBanner';
import { generateInsights, getTopInsights } from '../../../utils/insights';
...
function KPICard({ label, value, sub, icon: Icon, color, delta, sparkData }: {
  label: string; value: string; sub?: string; icon: React.ElementType; color: string; delta?: number; sparkData?: number[];
}) {
  return (
...
function InsightCard({
  label, value, description, icon: Icon, iconColor, children,
}: {
...
export default function Dashboard() {
  ...
  const kpisPrev      = useMemo(() => getKPIsMes(pedidos, getMesAnterior(mesFiltro)), [pedidos, mesFiltro]);
  const topInsights   = useMemo(() => getTopInsights(generateInsights({ pedidos, historico, produtos, configuracoes, mesAtual: mesFiltro })), [pedidos, historico, produtos, configuracoes, mesFiltro]);
  const capitalEstoque = useMemo(() => getCapitalEstoque(produtos), [produtos]);
  const projecao      = useMemo(() => getProjecaoMensal(kpis.faturamento, mesFiltro), [kpis.faturamento, mesFiltro]);
  ...
  const produtosParados  = produtosComStatus.filter((p) => p.vdDia === 0 && p.estoqueAtual > 0).length;
  ...
  const sparklines = useMemo(() => {
    return Array.from({ length: 30 }, (_, i) => {
      ...
      const dp = pedidos.filter((p) => p.data === iso && (p.status === 'Concluído' || p.status === 'Enviado'));
      return {
  ...
  const FERRAMENTAS = [
    { to: '/analise',    label: 'Análise',    desc: 'DRE por período',      icon: CalendarSearch, bg: 'bg-blue-50 dark:bg-blue-950/30',       fg: 'text-blue-600 dark:text-blue-400'      },
    { to: '/previsao',   label: 'Previsão',   desc: 'Forecast de receita',  icon: Telescope,      bg: 'bg-sky-50 dark:bg-sky-950/30',         fg: 'text-sky-600 dark:text-sky-400'        },
    { to: '/abc',        label: 'Curva ABC',  desc: 'Ranking de produtos',  icon: PieChart,       bg: 'bg-emerald-50 dark:bg-emerald-950/30', fg: 'text-emerald-600 dark:text-emerald-400'},
    { to: '/simulador',  label: 'Simulador',  desc: 'Cenários de preço',    icon: FlaskConical,   bg: 'bg-amber-50 dark:bg-amber-950/30',     fg: 'text-amber-600 dark:text-amber-400'    },
    { to: '/saude',      label: 'Saúde',      desc: 'Score do negócio',     icon: HeartPulse,     bg: 'bg-rose-50 dark:bg-rose-950/30',       fg: 'text-rose-600 dark:text-rose-400'      },
    { to: '/mapa-calor', label: 'Mapa Calor', desc: 'Padrões diários',      icon: Activity,       bg: 'bg-cyan-50 dark:bg-cyan-950/30',       fg: 'text-cyan-600 dark:text-cyan-400'      },
    { to: '/categorias', label: 'Categorias', desc: 'Por grupo de produto', icon: Bookmark,       bg: 'bg-teal-50 dark:bg-teal-950/30',       fg: 'text-teal-600 dark:text-teal-400'      },
    { to: '/anual',      label: 'Ano a Ano',  desc: 'Comparativo anual',    icon: CalendarRange,  bg: 'bg-pink-50 dark:bg-pink-950/30',       fg: 'text-pink-600 dark:text-pink-400'      },
  ];
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-1">
          ...
          {isCurrentMonth && (
            <span className="text-xs bg-core-green/10 dark:bg-core-green/10 text-core-green dark:text-core-green font-medium px-2.5 py-1 rounded-full ml-1">
              Mês atual
            </span>
          )}
        </div>
      ...
      <InsightsBanner insights={topInsights} />
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="card p-4 flex-1 min-w-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-0.5">
              ...
              <Link to="/calendario" className="text-xs text-core-green hover:text-core-green font-medium ml-2">Ver tudo →</Link>
            </div>
          </div>
          {(() => {
        ...
        <div className="card p-4 sm:w-64 flex flex-col gap-2 self-start">
          ...
          <Link to="/calendario" className="text-xs text-core-green hover:text-core-green font-medium mt-1">
            Ver calendário →
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        <KPICard label="Faturamento"       value={fmt(kpis.faturamento)}   icon={DollarSign}  color="bg-core-green"   delta={calcDelta(kpis.faturamento,   kpisPrev.faturamento)} sparkData={sparklines.map((d) => d.receita)} />
        <KPICard label="Pedidos"           value={fmtNum(kpis.pedidosMes)} icon={ShoppingCart} color="bg-blue-500"    delta={calcDelta(kpis.pedidosMes,    kpisPrev.pedidosMes)}   sub="concluídos + enviados" sparkData={sparklines.map((d) => d.pedidos)} />
        <KPICard label="Lucro Operacional" value={fmt(kpis.lucroOp)}       icon={TrendingUp}  color="bg-emerald-500"  delta={calcDelta(kpis.lucroOp,       kpisPrev.lucroOp)} sparkData={sparklines.map((d) => d.lucro)} />
        <KPICard label="Despesas Op."      value={fmt(despesasDoMes)}      icon={Target}      color="bg-slate-400"    sub={despesasDoMes === 0 ? 'não lançadas' : undefined} />
        <KPICard label="Ticket Médio"      value={fmt(kpis.ticket)}        icon={Tag}         color="bg-amber-500"    delta={calcDelta(kpis.ticket,        kpisPrev.ticket)} sparkData={sparklines.map((d) => d.ticket)} />
        <KPICard label="Lucro Líquido"     value={fmt(kpis.lucroLiquido)}  icon={TrendingUp}  color="bg-emerald-600"  delta={calcDelta(kpis.lucroLiquido,  kpisPrev.lucroLiquido)} />
      </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        >
          {produtosParados > 0 && (
            <p className="text-[10px] text-amber-500 font-medium mt-1">{produtosParados} SKUs parados sem venda</p>
          )}
        </InsightCard>
        {isCurrentMonth && projecao && projecao > 0 ? (
          ...
          <InsightCard
            ...
            icon={AlertTriangle}
            iconColor={produtosRuptura > 0 ? 'text-red-500' : produtosAcabando > 0 ? 'text-amber-500' : 'text-slate-400'}
          >
            <Link to="/estoque" className="text-[10px] text-core-green font-medium mt-2 inline-block">
              Ver estoque →
            </Link>
          </InsightCard>
        )}
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-slate-700 dark:text-slate-200 font-semibold text-sm">Ferramentas Rápidas</h2>
          <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">{FERRAMENTAS.length} ferramentas</span>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <p className="text-[9px] text-slate-400 dark:text-slate-500 leading-tight mt-0.5">{f.desc}</p>
                </div>
              </Link>
            );
          })}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <AlertCircle size={15} className={alertas.filter(a => a.severidade === 'critico').length > 0 ? 'text-red-500' : 'text-slate-300 dark:text-slate-600'} />
              <h2 className="text-slate-700 dark:text-slate-200 font-semibold text-sm">Alertas Ativos</h2>
              {alertas.length > 0 && (
            ...
            <Link to="/alertas" className="text-xs text-core-green font-medium flex items-center gap-1">
          ...
          {alertas.length === 0 ? (
            <div className="flex items-center gap-3 py-3">
              ...
              <p className="text-sm text-slate-400 dark:text-slate-500">Nenhum alerta ativo — tudo dentro dos parâmetros.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {alertas.slice(0, 4).map((a) => (
                <Link key={a.id} to={a.link ?? '/alertas'} className="block group">
                  <div className={`flex items-start gap-2.5 p-2.5 rounded-xl transition-colors ${
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-slate-700 dark:text-slate-200 truncate">{a.titulo}</p>
                      <p className="text-[11px] text-slate-400 dark:text-slate-500 truncate">{a.descricao}</p>
                    </div>
                    {a.valor && (
                      <span className="text-[11px] font-mono font-bold text-slate-500 dark:text-slate-400 flex-shrink-0">{a.valor}</span>
                    )}
                  </div>
                </Link>
              ))}
              {alertas.length > 4 && (
                <Link to="/alertas" className="text-xs text-slate-400 hover:text-core-green transition-colors pl-5">
        ...
        {(() => {
          ...
          const kpisAtual = isCurrentMonth ? kpis : getKPIsMes(pedidos, hoje.toISOString().slice(0, 7));
          const metas = [
            { label: 'Faturamento',  meta: cfg.metaFaturamento, atual: kpisAtual.faturamento, fmt: fmt },
            { label: 'Pedidos',      meta: cfg.metaPedidos,     atual: kpisAtual.pedidosMes,  fmt: (v: number) => String(Math.round(v)) },
            { label: 'Lucro Op.',    meta: cfg.metaLucro,       atual: kpisAtual.lucroOp,     fmt: fmt },
            { label: 'Margem',       meta: cfg.metaMargem,      atual: kpisAtual.margem,      fmt: (v: number) => `${v.toFixed(1)}%` },
            ...
            <div className="card p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-slate-700 dark:text-slate-200 font-semibold text-sm">Metas do Mês</h2>
                  <span className="text-[10px] text-slate-400 dark:text-slate-500">{diasRestantes}d restantes</span>
                </div>
                <Link to="/metas" className="text-xs text-core-green font-medium flex items-center gap-1">
              ...
              {metas.length === 0 ? (
                <div className="flex items-center gap-3 py-3">
                  ...
                  <div>
                    <p className="text-sm text-slate-400 dark:text-slate-500">Nenhuma meta definida.</p>
                    <Link to="/metas" className="text-xs text-core-green hover:underline">Definir metas →</Link>
                  </div>
                </div>
              ) : (
                <div className="space-y-3">
                  {metas.map(({ label, meta, atual, fmt: f }) => {
                    ...
                    const cor = pct >= 100 ? 'bg-emerald-500' : pct >= 75 ? 'bg-amber-400' : pct >= 50 ? 'bg-orange-400' : 'bg-red-400';
                    return (
                      <div key={label}>
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="text-slate-600 dark:text-slate-300 font-medium">{label}</span>
                          <span className="font-mono text-slate-500 dark:text-slate-400">
                            {f(atual)} <span className="text-slate-300 dark:text-slate-600">/</span> {f(meta!)}
                          </span>
                        </div>
                        ...
                        <p className="text-[10px] text-right text-slate-400 dark:text-slate-500 mt-0.5">{pct.toFixed(0)}%</p>
                      </div>
                    );
                  })}
                </div>
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={chartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="receita" name="Receita"   stroke={C.primary} strokeWidth={2} fill="url(#gradReceita)" />
              <Area type="monotone" dataKey="lucro"   name="Lucro Op." stroke={C.secondary} strokeWidth={2} fill="url(#gradLucro)"   />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/financeiro" className="text-xs text-core-green hover:text-core-green font-medium flex items-center gap-1">
        ...
        {chartAnual.length === 0 ? (
          <div className="h-40 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-slate-400 dark:text-slate-500 text-xs">
              Vá em <Link to="/financeiro" className="text-core-green hover:underline">Financeiro</Link> e clique em &ldquo;Lançar manualmente&rdquo; para registrar o histórico mensal.
            </p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={220}>
      ...
      <div className="card">
        <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
          ...
          {ranking.length > 0 && (
            <Link to="/vendas" className="text-xs text-core-green hover:text-core-green font-medium flex items-center gap-1">
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/estoque" className="text-xs text-core-green hover:text-core-green font-medium flex items-center gap-1">
        ...
        {produtos.length === 0 ? (
          <div className="py-8 text-center">
            ...
            <Link to="/configs" className="text-xs text-core-green hover:text-core-green mt-1 inline-block">
              Cadastrar produtos →
            </Link>
          </div>
        ) : (
              ...
              return (
                <div key={p.sku} className="relative group bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={
                      p.status === 'Estoque Estável' ? 'badge-ok' :
                      p.status === 'Estoque Baixo'   ? 'badge-critico' :
                      p.status === 'Estoque Acima'   ? 'badge-excesso' : 'badge-ruptura'
                    }>{p.status}</span>
                  </div>
