...
import {
  Megaphone, ChevronLeft, ChevronRight, Info,
  TrendingUp, TrendingDown, Minus,
} from 'lucide-react';
import { useStore } from '../../../store';
import {
  computeAcosAnalise, computeProjecao,
  type AcosRating, type AcosAnalise,
...
const RATING_META: Record<AcosRating, { label: string; bg: string; text: string; dot: string }> = {
  excelente: { label: 'Excelente',  bg: 'bg-emerald-100', text: 'text-emerald-700', dot: 'bg-emerald-500' },
...
function ChartTip({ active, payload, label }: { active?: boolean; payload?: TooltipPayloadItem[]; label?: string }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg px-3 py-2 text-xs">
      ...
      {d && <p className="text-slate-600 dark:text-slate-400">ACOS: <span className="font-bold text-slate-900 dark:text-white">{d.value.toFixed(1)}%</span></p>}
    </div>
  );
}
...
function Simulador({ analise }: { analise: AcosAnalise }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-4">Simulador de Projeção</p>
      ...
      {!temDados ? (
        ...
        <>
          <div className="flex items-center gap-4 mb-5">
            <span className="text-sm text-slate-600 dark:text-slate-300 shrink-0">Aumentar investimento em</span>
            <input
            ...
            />
            <span className="text-base font-bold text-slate-800 dark:text-slate-100 w-12 text-right shrink-0">+{delta}%</span>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3">
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Receita projetada</p>
              <p className="text-base font-bold text-slate-800 dark:text-slate-100">{fmt(proj.novaReceita)}</p>
            </div>
            <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3">
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Ganho de receita</p>
              <p className="text-base font-bold text-emerald-700 dark:text-emerald-400">+{fmt(proj.deltaReceita)}</p>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3">
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Novo gasto em ads</p>
              <p className="text-base font-bold text-slate-800 dark:text-slate-100">{fmt(proj.novoAds)}</p>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3">
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">ACOS permanece</p>
              <p className="text-base font-bold text-slate-800 dark:text-slate-100">{analise.acosAtual.toFixed(1)}%</p>
            </div>
          </div>

          <p className="text-xs text-slate-400 dark:text-slate-500 mt-3">
            Modelo: ROAS constante — assume que receita e ads crescem na mesma proporção. Não considera sazonalidade ou saturação de mercado.
          </p>
        </>
      )}
    </div>
...
export default function Ads() {
  ...
  const analise = useMemo(() => computeAcosAnalise({
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">ACOS Intelligence</h1>
          {analise.isMesAtivo && (
            <span className="text-xs bg-blue-100 text-blue-700 font-semibold px-2 py-0.5 rounded-full">ao vivo</span>
          )}
        </div>
      ...
      <div className="flex items-start gap-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl px-4 py-3 text-xs text-amber-800 dark:text-amber-300">
        ...
        <span>
          Investimento em ads estimado com base no percentual configurado ({configuracoes.percentualMarketing.toFixed(1)}% da receita).
          Para análise mais precisa, edite o histórico com os valores reais de cada mês em <strong>Financeiro → Fechar mês</strong>.
        </span>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-6">
        <div className="flex flex-col sm:flex-row sm:items-start gap-6">
          ...
          <div className="flex-1">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">ACOS / TACOS</p>
            {!temDados ? (
              ...
              <>
                <div className="flex items-baseline gap-3 mb-3">
                  ...
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${ratingMeta.bg} ${ratingMeta.text}`}>
                ...
                <div className="flex flex-wrap gap-4 text-sm">
                  <div>
                    <p className="text-xs text-slate-400 dark:text-slate-500 mb-0.5">Referência saudável</p>
                    <p className="font-semibold text-slate-700 dark:text-slate-200">≤ 10%</p>
                  </div>
                  {analise.hasHistorico && (
                    <div>
                      <p className="text-xs text-slate-400 dark:text-slate-500 mb-0.5">Sua média histórica</p>
                      <div className="flex items-center gap-1.5">
                        <span className="font-semibold text-slate-700 dark:text-slate-200">{analise.benchmarkInterno.toFixed(1)}%</span>
            ...
            <div className="w-full sm:w-48 h-20 shrink-0">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                  ...
                  <Area
                    type="monotone" dataKey="acos"
                    stroke="#10b981" strokeWidth={2}
                    fill="url(#acosGradMini)" dot={false}
                  />
                </AreaChart>
              </ResponsiveContainer>
              <p className="text-xs text-slate-400 text-center mt-1">últimos {chartData.length} meses</p>
            </div>
          )}
        </div>
      </div>
      ...
      {temDados && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <MetricCard title="Quanto custa vender">
            <p className="text-2xl font-black text-slate-800 dark:text-slate-100 mb-1">{fmt(analise.adsTotal)}</p>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">investido em ads no mês</p>
            <div className="space-y-1.5 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-700 dark:text-slate-200">R${analise.custoPorCem.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">Receita gerada</span>
                <span className="font-semibold text-slate-700 dark:text-slate-200">{fmt(analise.receitaTotal)}</span>
              </div>
            </div>
          </MetricCard>
          ...
          <MetricCard title="Quanto poderia investir">
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">Com ACOS ≤ 10% (referência saudável)</p>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">Máximo saudável</span>
                <span className="font-semibold text-slate-700 dark:text-slate-200">{fmt(analise.investimentoMaximo)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">Atual estimado</span>
                <span className="font-semibold text-slate-700 dark:text-slate-200">{fmt(analise.adsTotal)}</span>
              </div>
              <div className="border-t border-slate-100 dark:border-slate-700 pt-2 flex justify-between">
                <span className="font-semibold text-slate-600 dark:text-slate-300">Saldo disponível</span>
                <span className={`font-bold ${analise.saldoDisponivel >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                  {analise.saldoDisponivel >= 0 ? '+' : ''}{fmt(analise.saldoDisponivel)}
                </span>
              </div>
            </div>
          ...
          <MetricCard title={analise.hasHistorico ? 'Ganhou / Perdeu vs. histórico' : 'Histórico insuficiente'}>
            {!analise.hasHistorico ? (
              <p className="text-sm text-slate-400">É necessário pelo menos 2 meses fechados para calcular o benchmark interno.</p>
            ) : (
              <>
                <div className="flex items-baseline gap-2 mb-3">
                  <span className={`text-2xl font-black ${analise.custoExtraVsBenchmark > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                    {analise.custoExtraVsBenchmark > 0 ? '+' : ''}{fmt(analise.custoExtraVsBenchmark)}
                  </span>
                ...
                <div className="space-y-1.5 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-slate-700 dark:text-slate-200">{analise.acosAtual.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
      ...
      {chartData.length > 0 && (
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
          ...
          {chartData.length < 2 ? (
            <p className="text-sm text-slate-400 py-6 text-center">Feche pelo menos 2 meses para visualizar a evolução.</p>
          ) : (
            <>
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={chartData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                  ...
                  <ReferenceLine y={8}  stroke="#10b981" strokeDasharray="4 3" strokeWidth={1}
                    label={{ value: '8% excelente', position: 'insideTopRight', fontSize: 10, fill: '#10b981' }} />
                  <ReferenceLine y={10} stroke="#f59e0b" strokeDasharray="4 3" strokeWidth={1}
                    label={{ value: '10% referência', position: 'insideTopRight', fontSize: 10, fill: '#f59e0b' }} />
                  <ReferenceLine y={12} stroke="#ef4444" strokeDasharray="4 3" strokeWidth={1}
                    label={{ value: '12% crítico', position: 'insideTopRight', fontSize: 10, fill: '#ef4444' }} />
                  <Area
                    type="monotone" dataKey="acos"
                    stroke="#10b981" strokeWidth={2.5}
                    fill="url(#acosGrad)" dot={{ r: 3, fill: '#10b981', strokeWidth: 0 }}
                    activeDot={{ r: 5 }}
                  />
                </AreaChart>
              </ResponsiveContainer>
              ...
              <div className="flex flex-wrap gap-4 mt-3 justify-center text-xs text-slate-500">
                <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-emerald-500 inline-block" />≤8% Excelente</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-amber-400 inline-block" />≤10% Bom</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-red-400 inline-block" />&gt;12% Crítico</span>
              </div>
            </>
          )}
        </div>
