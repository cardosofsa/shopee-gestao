...
const SEG_CONFIG: Record<
  Segmento,
  { label: string; icon: React.ElementType; badge: string; dot: string; color: string }
> = {
  VIP: {
    label: 'VIP',
    icon: Trophy,
...
function construirPerfis(pedidos: Pedido[]): ClienteProfile[] {
  ...
  const concluidos = (ps: Pedido[]) =>
    ps.filter((p) => p.status === 'Concluído' || p.status === 'Enviado');
...
function ClienteModal({ cliente, onClose }: { cliente: ClienteProfile; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[88vh] overflow-hidden flex flex-col"
          ...
          <div className="px-6 py-3 border-b border-slate-100 dark:border-slate-700">
            ...
            <div className="flex flex-wrap gap-2">
              {topProdutos.map((p) => (
                <div
                  key={p.nome}
                  className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 dark:bg-slate-700 rounded-lg"
                >
                  <span className="text-xs font-medium text-slate-700 dark:text-slate-200 truncate max-w-32">
                    {p.nome}
                  </span>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
              {cliente.pedidos.map((p) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-slate-700 dark:text-slate-200 max-w-[180px] truncate">
                    {p.produto}
                  </td>
...
export default function Clientes() {
  ...
  const [sortCol, setSortCol] = useState<'faturamento' | 'pedidos' | 'ticket' | 'diasDesdeUltimo'>(
    'faturamento'
  );
  ...
  const kpis = useMemo(() => {
    ...
    const porSeg = Object.fromEntries(
      SEGMENTOS_ORDEM.map((s) => [s, perfis.filter((p) => p.segmento === s).length])
    );
  ...
  const SortIcon = ({ col }: { col: typeof sortCol }) =>
    sortCol !== col ? null : sortDir === 'asc' ? (
      <ChevronUp size={11} className="text-core-green" />
    ) : (
      <ChevronDown size={11} className="text-core-green" />
    );
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            Segmentação, LTV e histórico · extraído dos pedidos
          </p>
        </div>
        {temNomes && (
      ...
      {!temNomes && (
        <div className="card p-12 text-center">
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm max-w-sm mx-auto">
            Os dados são extraídos automaticamente ao importar CSVs do Shopee Seller Center. O campo{' '}
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div className="card p-4">
              ...
              <p className="text-2xl font-bold text-amber-500">
                {kpis.ltvVip > 0 ? fmt(kpis.ltvVip) : '—'}
              </p>
              <p className="text-xs text-slate-400 mt-0.5">
                {kpis.porSeg['VIP'] ?? 0} clientes VIP
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {SEGMENTOS_ORDEM.map((seg) => {
              ...
              return (
                <button
                  key={seg}
                  onClick={() => setSegFiltro(segFiltro === seg ? null : seg)}
                ...
                >
                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${dot}`} />
                  {label} ({count}){segFiltro === seg && <X size={10} />}
                </button>
              );
            })}
          </div>
                  ...
                  {filtrados.length === 0 ? (
                    ...
                    filtrados.map((c) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-bold text-slate-800 dark:text-slate-100">
                          ...
                          {c.recorrente && (
                            <span title="Recorrente">
                              <RefreshCw size={10} className="inline ml-1 text-emerald-400" />
                            </span>
                          )}
                        </td>
                        ...
                        <td
                          className={`px-4 py-3 font-medium ${c.margem >= 20 ? 'text-emerald-600' : c.margem >= 10 ? 'text-amber-500' : 'text-red-500'}`}
                        >
                          {fmtPct(c.margem)}
                        </td>
                        <td className="px-4 py-3">
                          ...
                          <p
                            className={`text-[10px] ${c.diasDesdeUltimo > 60 ? 'text-red-400' : c.diasDesdeUltimo > 30 ? 'text-amber-500' : 'text-slate-400'}`}
                          >
              ...
              <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex justify-between text-xs text-slate-400 dark:text-slate-500">
                ...
                <span>
                  ...
                  <span className="font-medium text-slate-600 dark:text-slate-300">
                    {fmt(filtrados.reduce((s, c) => s + c.faturamento, 0))}
                  </span>
                </span>
              </div>
            )}
