import { Download, Pencil, Plus, SlidersHorizontal, Trash2, X } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';

import { useToast } from '../../../components/Toast';
import { useStore } from '../../../store';
import type { AjusteEstoque, Compra, PrioridadeTarefa } from '../../../types';
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-sm">
        <div className="px-6 py-5 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            O estoque do SKU será revertido automaticamente.
          </p>
        </div>
...
function AjusteModal({ onClose }: { onClose: () => void }) {
  ...
  const save = () => {
    ...
    const ajuste: AjusteEstoque = {
      ...
      estoqueDepois: estoqueResultante,
      motivo: form.motivo,
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, quantidade: Math.max(1, parseInt(e.target.value) || 1) }))
              }
            />
          </div>
          <div>
          ...
          {prod && (
            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 flex items-center justify-between text-sm">
              ...
              <div className="text-center">
                ...
                <p
                  className={`font-bold text-lg ${form.tipo === 'entrada' ? 'text-emerald-600' : 'text-red-500'}`}
                >
                  {estoqueResultante}
                </p>
              </div>
...
function AddCompraModal({ onClose }: { onClose: () => void }) {
  ...
  const [cab, setCab] = useState({
    ...
    pagamento: 'Pix',
    parcelas: 1,
    loja: configuracoes.lojas[0] ?? 'Ambas',
    observacoes: '',
  });
  ...
  const fc =
    ...
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) =>
      setCab((prev) => ({
        ...prev,
        [k]: e.target.type === 'number' ? parseInt(e.target.value) || 1 : e.target.value,
      }));
    ...
    setLinhas((prev) => {
      ...
      if (field === 'sku') {
        ...
        next[i] = {
          ...next[i],
          sku: raw,
          custoUnitario: prod?.custoUnitario ?? next[i].custoUnitario,
        };
      } else {
  ...
  const save = () => {
    ...
    linhas.forEach((l) => {
      ...
      addCompra({
        ...
        sku: l.sku,
        produto: prod?.nome ?? '',
        quantidadeEntrada: l.quantidadeEntrada,
        custoUnitario: l.custoUnitario,
        custoTotal: custoTot,
        valorParcela: custoTot / Math.max(1, cab.parcelas),
      });
      if (prod && Math.abs(prod.custoUnitario - l.custoUnitario) > 0.001) {
        diffs.push({
          sku: l.sku,
          nome: prod.nome,
          custoAnterior: prod.custoUnitario,
          custoNovo: l.custoUnitario,
        });
      }
    });
    toast(
      linhas.length === 1
        ? `Compra de ${linhas[0].quantidadeEntrada} un. de ${linhas[0].sku} registrada.`
        : `${linhas.length} itens registrados na nota.`,
      'success'
    );
    if (diffs.length > 0) {
      setCustoDiffs(diffs);
      ...
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md">
          <div className="px-6 py-5 text-center">
            ...
            <h3 className="font-semibold text-slate-900 dark:text-slate-100 mb-1">
              Custo diferente detectado
            </h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-4">
              Deseja atualizar o custo cadastrado dos seguintes SKUs?
            </p>
            <div className="space-y-2 text-left">
              {custoDiffs.map((d) => (
                <div
                  key={d.sku}
                  className="bg-slate-50 dark:bg-slate-800/50 rounded-xl px-4 py-3 text-sm flex items-center justify-between"
                >
                  <div>
                  ...
                  <div className="text-right">
                    <p className="text-slate-400 dark:text-slate-500 text-xs line-through">
                      {fmt(d.custoAnterior)}
                    </p>
          ...
          <div className="px-6 pb-5 flex gap-3">
            ...
            <button className="btn-primary flex-1 justify-center" onClick={applyDiffs}>
              Sim, atualizar
            </button>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-2xl max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-6 pb-0 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <input type="date" className="input" value={cab.data} onChange={fc('data')} />
          </div>
          <div>
          ...
          <div>
            ...
            <input className="input" value={cab.nfRef} onChange={fc('nfRef')} />
          </div>
          <div>
            <label className="text-xs text-slate-500 dark:text-slate-400 mb-1 block">
              Pagamento
          ...
          <div className="col-span-2">
            ...
            <input className="input" value={cab.observacoes} onChange={fc('observacoes')} />
          </div>
        </div>
        <div className="px-6 py-4">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">Itens</p>
            <button onClick={addLinha} className="btn-secondary py-1 px-3 text-xs">
          ...
          <div className="space-y-2">
            {linhas.map((l, i) => {
              ...
              return (
                <div
                  key={i}
                  className="grid grid-cols-[1fr_80px_90px_80px_auto] gap-2 items-center bg-slate-50 dark:bg-slate-700/50 rounded-xl px-3 py-2.5"
                >
                  <div>
                    <select
                      className="select text-xs py-1.5"
                      value={l.sku}
                      onChange={(e) => updateLinha(i, 'sku', e.target.value)}
                    >
                      {produtos.map((p) => (
                        <option key={p.sku} value={p.sku}>
                          {p.sku} — {p.nome}
                    ...
                    {prod && (
                      <p className="text-[10px] text-slate-400 mt-0.5 pl-0.5">
                        Custo cadastrado: {fmt(prod.custoUnitario)} · Estoque: {prod.estoqueAtual}{' '}
                        un.
                      </p>
                    )}
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      value={l.quantidadeEntrada}
                      onChange={(e) => updateLinha(i, 'quantidadeEntrada', e.target.value)}
                    />
                  </div>
                  <div>
                    <label className="text-[10px] text-slate-400 block mb-0.5">Custo unit.</label>
                    <input
                      type="number"
                      step="0.01"
                      className="input text-xs py-1.5"
                      value={l.custoUnitario}
                      onChange={(e) => updateLinha(i, 'custoUnitario', e.target.value)}
                    />
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-slate-400 mb-0.5">Subtotal</p>
                    <p className="text-xs font-bold text-slate-800 dark:text-slate-100">
                      {fmt(l.quantidadeEntrada * l.custoUnitario)}
                    </p>
                  </div>
                  <button
          ...
          <div className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-3 flex gap-5 text-sm flex-wrap">
            ...
            {cab.parcelas > 1 && (
              <span className="text-slate-400 dark:text-slate-500">
                {cab.parcelas}× de {fmt(custoGeral / cab.parcelas)}
              </span>
            )}
          </div>
        </div>
        <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3">
          ...
          <button className="btn-primary" onClick={save}>
            Salvar {linhas.length > 1 ? `${linhas.length} itens` : 'Compra'}
          </button>
        </div>
      </div>
    </div>
...
function EditCompraModal({ compra, onClose }: { compra: Compra; onClose: () => void }) {
    ...
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
      ...
      if (k === 'sku') {
        ...
        setForm((prev) => ({
          ...prev,
          sku: e.target.value,
          custoUnitario: p?.custoUnitario ?? prev.custoUnitario,
        }));
      } else {
  ...
  const save = () => {
    updateCompra(compra.id, {
      ...form,
      custoTotal: custoTot,
      valorParcela: custoTot / Math.max(1, form.parcelas),
    });
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-sm">
          <div className="px-6 py-5 text-center">
            ...
            <p className="text-slate-400 dark:text-slate-500 text-xs mb-3">
              Deseja atualizar o custo cadastrado de{' '}
          ...
          <div className="px-6 pb-5 flex gap-3">
            ...
            <button
              ...
              onClick={() => {
                updateProduto(custoDiff.sku, { custoUnitario: custoDiff.custoNovo });
                toast('Compra e custo atualizados.', 'success');
                onClose();
              }}
            >
              Sim, atualizar
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-lg max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-6 grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="input" value={form.fornecedor} onChange={f('fornecedor')} />
          </div>
          <div>
            <label className="text-xs text-slate-500 dark:text-slate-400 mb-1 block">
              NF / Ref.
            </label>
            <input className="input" value={form.nfRef} onChange={f('nfRef')} />
          </div>
          <div>
            <label className="text-xs text-slate-500 dark:text-slate-400 mb-1 block">
              Pagamento
          ...
          <div className="col-span-2 bg-slate-50 dark:bg-slate-700/50 rounded-xl p-3 text-sm flex gap-5">
            <span className="text-slate-500 dark:text-slate-400">
              Custo Total:{' '}
              <span className="font-bold text-slate-800 dark:text-slate-100">{fmt(custoTot)}</span>
            </span>
            {form.parcelas > 1 && (
              <span className="text-slate-400 dark:text-slate-500">
                {form.parcelas}× de {fmt(custoTot / form.parcelas)}
              </span>
            )}
            {prod && (
              <span className="text-slate-500 dark:text-slate-400">
                ...
                <span className="font-bold text-emerald-600">
                  {prod.estoqueAtual + (form.quantidadeEntrada - compra.quantidadeEntrada)} un.
                </span>
              </span>
            )}
          </div>
...
function AddProdutoModal({ onClose }: { onClose: () => void }) {
  const toast = useToast();
  const produtos = useStore((s) => s.produtos);
    ...
    const val =
      e.target.type === 'number'
        ...
        : e.target.type === 'checkbox'
          ...
          : e.target.value;
    setForm((prev) => ({ ...prev, [k]: val }));
    if (k === 'sku') setSkuError('');
  };

  const save = () => {
    if (!form.sku.trim()) return setSkuError('SKU obrigatório.');
    if (!form.nome.trim()) return setSkuError('Nome obrigatório.');
    if (produtos.some((p) => p.sku === form.sku.trim())) return setSkuError('SKU já existe.');
    addProduto({
      ...form,
      sku: form.sku.trim(),
      nome: form.nome.trim(),
    ...
    toast(`Produto ${form.sku.trim()} criado.`, 'success');
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md">
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
          <h3 className="font-semibold text-slate-900 dark:text-slate-100">Novo Produto</h3>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-600 transition-colors"
          >
        ...
        <div className="p-6 space-y-4">
          ...
          {skuError && <p className="text-xs text-red-500 -mt-2">{skuError}</p>}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-slate-500 dark:text-slate-400 mb-1 block">
                Categoria
...
export default function Estoque() {
  ...
  const estoqueData = useMemo((): EstoqueItem[] => {
    ...
    return produtos.map((p) => {
      ...
      const entradas = compras
        ...
        .reduce((s, c) => s + c.quantidadeEntrada, 0);
      return {
        ...p,
        vendaDia,
        diasCobertura,
        ptReposicao,
        status,
        valorEstoque: p.estoqueAtual * p.custoUnitario,
        entradas,
        saidas: totalUnid,
      };
    });
  ...
  const coberturaMedia = useMemo(() => {
    ...
    return Math.round(
      comVenda.reduce((s, p) => s + (p.diasCobertura as number), 0) / comVenda.length
    );
  }, [estoqueData]);
  ...
  const chartData = useMemo(() => {
    ...
    return [...estoqueData]
      ...
      .map((p) => ({
        ...
        pct: (p.valorEstoque / totalEstoque) * 100,
        status: p.status,
      }));
    ...
    exportXlsx(`estoque_${new Date().toISOString().slice(0, 10)}`, [
      {
        ...
        rows: estoqueData.map((p) => [
          ...
          p.status,
          xlsxNum(p.vendaDia),
          isFinite(p.diasCobertura) ? p.diasCobertura : '',
          xlsxNum(p.custoUnitario),
          xlsxNum(p.valorEstoque),
        ]),
      },
    ]);
    ...
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            Posição em tempo real · últimos 30 dias
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <button className="btn-secondary" onClick={() => setShowAddProduto(true)}>
      ...
      {showAddProduto && <AddProdutoModal onClose={() => setShowAddProduto(false)} />}
      ...
      {editProdutoSku && (
        <EditProdutoModal sku={editProdutoSku} onClose={() => setEditProdutoSku(null)} />
      )}
      {deleteId && (
        <ConfirmDeleteModal
          onConfirm={() => {
            deleteCompra(deleteId);
            toast('Compra excluída e estoque revertido.', 'info');
            setDeleteId(null);
          }}
