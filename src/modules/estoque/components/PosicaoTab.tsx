...
import { Search, X, ChevronUp, ChevronDown, ChevronsUpDown, Pencil, Package, ExternalLink } from 'lucide-react';
        ...
        <div className="card p-4">
          ...
          <ResponsiveContainer width="100%" height={chartData.length * 32 + 8}>
            <BarChart data={chartData} layout="vertical" margin={{ top: 0, right: 56, left: 0, bottom: 0 }}>
              ...
              <YAxis dataKey="sku" type="category" width={96} tick={{ fontSize: 11, fontFamily: 'monospace', fill: C.slate }} />
              ...
              <Bar dataKey="valor" radius={[0, 4, 4, 0]}
                label={{
                  content: (props: any) => {
                    ...
                    return (
                      <text x={x + width + 4} y={y + height / 2} fontSize={11} fill={C.slate} dominantBaseline="middle">
                        {pct.toFixed(0)}%
                      </text>
                    );
                  },
                  ...
                  <tr key={p.sku} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors">
                    ...
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-2">
                        <Link to={`/estoque/${p.sku}`}
                          className="text-slate-300 hover:text-core-green transition-colors" title="Ver detalhes">
                          <ExternalLink size={13} />
                        </Link>
                        <button onClick={() => onEditProduto(p.sku)}
                          className="text-slate-300 hover:text-core-green transition-colors" title="Editar produto">
                          <Pencil size={13} />
                        </button>
                        <button onClick={() => onAjuste()}
                          className="text-slate-300 hover:text-core-green transition-colors" title="Ajustar estoque">
                          <Package size={13} />
                        </button>
                      </div>
                    </td>
