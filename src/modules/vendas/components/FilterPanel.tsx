...
export function FilterPanel({
  ...
  return (
    <div
      className={`overflow-hidden transition-all duration-200 ease-in-out ${open ? 'max-h-[520px] opacity-100' : 'max-h-0 opacity-0'}`}
    >
          ...
          <div className="space-y-2">
            {STATUS_OPTIONS.map((s) => (
              <label key={s} className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span
                  className={`inline-flex items-center gap-1.5 text-xs font-medium px-2 py-0.5 rounded-full border ${STATUS_STYLE[s].badge}`}
                >
