...
import { Plus, Trash2, RotateCcw, Tag, CheckCircle2, Download, Wifi, WifiOff, Zap, Crown, Users, CalendarCheck, Link2, Link2Off, X, Store } from 'lucide-react';
...
export default function Configs() {
  ...
  const [newProd, setNewProd] = useState<Partial<Produto>>({
    sku: '', nome: '', categoria: 'Perfumaria', loja: configuracoes.lojas[0] ?? 'Ambas',
  ...
  const addProd = () => {
    ...
    setNewProd({ sku: '', nome: '', categoria: 'Perfumaria', loja: configuracoes.lojas[0] ?? 'Ambas', custoUnitario: 0, estoqueSeguranca: 10, estoqueAtual: 0, ativo: true });
    setShowAdd(false);
  };
    ...
    <div className="p-6 space-y-6">
      ...
      {(() => {
        ...
        const PLAN_STYLES: Record<string, { badge: string; icon: React.ReactNode }> = {
          ...
          pro:             { badge: 'bg-core-green/5 text-core-green border-core-green/20',          icon: <Crown size={14} /> },
      ...
      <div className="card p-5 space-y-4">
        ...
        <p className="text-slate-400 dark:text-slate-500 text-xs">Nomes das lojas usados para filtrar vendas, despesas e compras. "Ambas" é reservado e não precisa ser cadastrado.</p>
        <div className="flex flex-wrap gap-2">
          {configuracoes.lojas.map((l) => (
            <span key={l} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-medium">
              {l}
              {configuracoes.lojas.length > 1 && (
                <button
                  className="text-slate-400 hover:text-red-500 transition-colors"
                  onClick={() => {
                    const next = { ...configuracoes, lojas: configuracoes.lojas.filter((x) => x !== l) };
                    setCfg(next);
                    updateConfiguracoes(next);
                  }}
                >
        ...
        <table className="text-sm w-full max-w-lg">
          ...
          <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
            {[
              ...
              return (
                <tr key={r.faixa} className={isCurrent ? 'bg-core-green/5 dark:bg-core-green/10' : 'hover:bg-slate-50 dark:hover:bg-slate-700/20'}>
                  <td className="py-2 pl-2 text-slate-600 dark:text-slate-300 font-medium">
                    {r.faixa}
                    {isCurrent && <span className="ml-2 text-[10px] bg-core-green/10 text-core-green px-1.5 py-0.5 rounded font-semibold">atual</span>}
                  </td>
                  ...
                  <td className={`py-2 font-semibold ${isCurrent ? 'text-core-green' : 'text-slate-500 dark:text-slate-400'}`}>{r.aliq}</td>
                </tr>
              );
            })}
          </tbody>
                ...
                <tr key={p.sku} className={`hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors group ${!p.ativo ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <select className="select py-1 text-xs" value={p.loja}
                      ...
                      {[...new Set([p.loja, ...configuracoes.lojas, 'Ambas'])].map((l) => <option key={l}>{l}</option>)}
                    </select>
                  </td>
                  <td className="px-4 py-3">
