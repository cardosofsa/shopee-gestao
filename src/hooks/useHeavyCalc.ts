...
import type { Insight, InsightInput } from '../utils/insights';
...
function getWorker(): Worker {
  ...
  if (!w[key]) {
    w[key] = new Worker(new URL('../workers/calculations.worker.ts', import.meta.url), {
      type: 'module',
    });
  }
...
export function useHeavyCalc() {
  ...
  const computeInsights = useCallback((input: InsightInput): Promise<Insight[]> => {
  ...
  const computeRanking = useCallback((pedidos: Pedido[]): Promise<RankingProduto[]> => {
