  ...
  return useMemo(() => {
    ...
    for (const prod of produtosAll.filter((p) => p.ativo)) {
      ...
      if (prod.estoqueAtual <= 0) {
        alertas.push({
          id: `est-zero-${prod.sku}`,
          severidade: 'critico',
          categoria: 'estoque',
          titulo: `Estoque zerado — ${prod.nome}`,
          descricao: `SKU ${prod.sku} · sem unidades disponíveis para venda`,
          link: `/estoque/${prod.sku}`,
          valor: '0 un.',
        });
      } else if (
        (prod.estoqueAtual <= prod.estoqueSeguranca && prod.estoqueSeguranca > 0) ||
        diasCob < 5
      ) {
        alertas.push({
          id: `est-baixo-${prod.sku}`,
          severidade: 'aviso',
          categoria: 'estoque',
    ...
    if (aliquotaDAS > 0) {
      ...
      if (diasParaDAS >= 0 && diasParaDAS <= 7 && fatMesAnt > 0) {
        alertas.push({
          id: 'das-vencer',
          severidade: diasParaDAS <= 3 ? 'critico' : 'aviso',
          categoria: 'fiscal',
    ...
    if (kpis.pedidosMes > 10 && kpis.margem < 10) {
      alertas.push({
        id: 'margem-mes',
        severidade: kpis.margem < 0 ? 'critico' : 'aviso',
    ...
    for (const c of contasPagar.filter((c) => c.status === 'pendente')) {
      ...
      const diff = Math.ceil(
        (new Date(c.vencimento + 'T00:00:00').getTime() -
