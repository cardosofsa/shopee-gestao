...
if (SERVICE_KEY && jwtRole(SERVICE_KEY) !== 'service_role') {
  console.error(
    `❌  A key configurada tem role="${jwtRole(SERVICE_KEY)}" — não é a service_role key.`
  );
...
async function resolveUser() {
  ...
  if (alreadyExists) {
    ...
    if (found) {
      console.log(`♻️  Usuário existente: ${found.id}`);
      return found.id;
    }
  }
  ...
  if (su?.user && !e2) {
    ...
    try {
      await db.auth.admin.updateUserById(su.user.id, { email_confirm: true });
    } catch {}
...
const FORNECEDORES = [
  {
    nome: 'Distribuidora Alfa Beleza',
    telefone: '(11) 91234-5678',
    email: 'compras@alfa.com.br',
    leadTimeDias: 7,
    termosPagamento: '30/60 dias',
    observacoes: 'Fornecedor principal de perfumaria e cosméticos',
  },
  {
    nome: 'Tech Import Brasil',
    telefone: '(11) 98765-4321',
    email: 'vendas@techimport.com.br',
    leadTimeDias: 10,
    termosPagamento: 'À vista (Pix 3%)',
    observacoes: 'Cabos, carregadores e acessórios eletrônicos',
  },
  {
    nome: 'Casa & Decoração LTDA',
    telefone: '(11) 93322-1100',
    email: 'pedidos@casadec.com.br',
    leadTimeDias: 5,
    termosPagamento: '15/30 dias',
    observacoes: 'Organização, bambu e itens de casa',
  },
  {
    nome: 'Prime Saúde Distribuidora',
    telefone: '(11) 94455-6677',
    email: 'prime@primesaude.com.br',
    leadTimeDias: 8,
    termosPagamento: '30 dias',
    observacoes: 'Equipamentos e produtos de saúde',
  },
  {
    nome: 'Fashion Acessórios SP',
    telefone: '(11) 99988-7766',
    email: 'fash@fashionaces.com.br',
    leadTimeDias: 6,
    termosPagamento: '30/60 dias',
    observacoes: 'Bolsas, carteiras, relógios e acessórios moda',
  },
];
...
const TAREFAS = [
  {
    titulo: 'Repor Suporte Celular Veicular (RUPTURA)',
    descricao: 'CS-ELET-009 com apenas 2 unidades — fazer pedido urgente à Tech Import',
    coluna: 'todo',
    posicao: 0,
    prioridade: 'alta',
  ...
  {
    titulo: 'Repor Creme Hidratante Corporal',
    descricao: 'CS-PERF-015 com 8 unidades (seg=30) — solicitar 100 unidades à Alfa',
    coluna: 'todo',
    posicao: 1,
    prioridade: 'alta',
  ...
  {
    titulo: 'Criar campanha Dia dos Pais (Agosto)',
    descricao: 'Planejar campanha com desconto 12% nos kits de perfumaria',
    coluna: 'todo',
    posicao: 2,
    prioridade: 'media',
  ...
  {
    titulo: 'Negociar frete com Alfa Beleza',
    descricao: 'Volume atual justifica revisão de termos — reunião solicitada',
    coluna: 'todo',
    posicao: 3,
    prioridade: 'media',
    dataVencimento: null,
  ...
  {
    titulo: 'Revisar ads — ACOS >12% em Abril e Maio',
    descricao: 'Revisar segmentação das campanhas Shopee Ads para reduzir gasto',
    coluna: 'in_progress',
    posicao: 0,
    prioridade: 'alta',
  ...
  {
    titulo: 'Atualizar fotos do Fone Bluetooth TWS',
    descricao: 'CS-ELET-010 — novas fotos com fundo branco e dimensões corretas',
    coluna: 'in_progress',
    posicao: 1,
    prioridade: 'media',
    dataVencimento: null,
  ...
  {
    titulo: 'Fechar DRE de Maio 2026',
    descricao: 'Conferir despesas e fechar no Financeiro — meta não atingida (R$42k vs R$50k)',
    coluna: 'in_progress',
    posicao: 2,
    prioridade: 'alta',
  ...
  {
    titulo: 'Importar pedidos de Maio 2026',
    descricao: 'Pedidos já importados via planilha Shopee',
    coluna: 'done',
    posicao: 0,
    prioridade: 'alta',
    dataVencimento: null,
  ...
  {
    titulo: 'Cadastrar produtos CORE Premium',
    descricao: 'Todos os 12 SKUs premium já cadastrados e com estoque',
    coluna: 'done',
    posicao: 1,
    prioridade: 'media',
    dataVencimento: null,
  ...
  {
    titulo: 'Configurar alíquota DAS e marketing',
    descricao: 'Configurado: DAS 6%, Marketing 2%',
    coluna: 'done',
    posicao: 2,
    prioridade: 'media',
    dataVencimento: null,
...
const CONTAS_DEF = [
  {
    descricao: 'Embalagens — Fatura Junho',
  ...
  {
    descricao: 'Aluguel Galpão Estoque',
    categoria: 'Fixo',
    valor: 1200,
  ...
  {
    descricao: 'Shopee Gestão — Plano Pro',
    categoria: 'Software',
    valor: 89,
  ...
  {
    descricao: 'Ads Shopee Premium — Junho',
    categoria: 'Marketing',
    valor: 600,
    vencimento: '2026-06-28',
    status: 'pendente',
  ...
  {
    descricao: 'Frete Correios — Contrato',
    categoria: 'Logística',
    valor: 350,
  ...
  {
    descricao: 'Contador MEI — Honorários',
    categoria: 'Contábil',
    valor: 200,
...
const HISTORICO_DEF = [
  ...
  }, // ACOS 15%
  {
    ...
    ads: 6300,
    desp: 2200,
  }, // ACOS 15%, meta R$50k NÃO ATINGIDA
];
...
function priceFromCusto(custo) {
  ...
  return Math.ceil(raw / 0.1) * 0.1 - 0.01; // arredonda para .99 ou .90
}
...
function calcOrder(custo, qty, monthYM, basePrice) {
  ...
  const taxaShopee = parseFloat((receita * 0.2).toFixed(2));
  ...
  const lucroOp = parseFloat(
    (receita - desconto - custoTotal - taxaShopee - dasImposto - adsMarketing).toFixed(2)
  );
  const margemSCustoProduto =
    receita > 0
      ? parseFloat(
          (((receita - desconto - taxaShopee - dasImposto - adsMarketing) / receita) * 100).toFixed(
            2
          )
        )
      : 0;
  const margemSCustoTotal = receita > 0 ? parseFloat(((lucroOp / receita) * 100).toFixed(2)) : 0;
  return {
    receita,
    desconto,
    custoTotal,
...
function generateOrders(uid, products) {
  ...
  for (const { ym, count } of MONTH_ORDERS) {
    ...
    for (let i = 0; i < totalCount; i++) {
      ...
      const date = `${ym}-${day}`;
      const loja = prod.loja === 'Ambas' ? (rng() < 0.7 ? LOJA_A : LOJA_B) : prod.loja;
...
async function main() {
  ...
  const products = PRODUCTS_DEF.map(([sku, nome, categoria, loja, custo, seg, atual, demanda]) => ({
    ...
    loja,
    custoUnitario: custo,
    estoqueSeguranca: seg,
    estoqueAtual: atual,
    ativo: true,
    demanda,
    basePrice: priceFromCusto(custo),
  }));
  ...
  }));
  {
    ...
    if (error) {
      console.error('❌ produtos:', error.message);
      process.exit(1);
    }
  }
  ...
  for (let i = 0; i < orders.length; i += 200) {
    ...
    if (error) {
      console.error('❌ pedidos (lote):', error.message);
      process.exit(1);
    }
  ...
  const COMPRAS = [
    {
      sku: 'CS-PERF-010',
      produto: 'Body Splash Ocean 200ml',
      data: '2026-05-10',
      qtd: 100,
      custo: 10,
      forn: 'Distribuidora Alfa Beleza',
      pag: 'Pix',
    },
    {
      sku: 'CS-ELET-001',
      produto: 'Cabo USB-C para USB-C 1m Nylon',
      data: '2026-05-15',
      qtd: 200,
      custo: 5,
      forn: 'Tech Import Brasil',
      pag: 'Boleto 30d',
    },
    {
      sku: 'CS-ELET-006',
      produto: 'Carregador Rápido 20W USB-C',
      data: '2026-05-15',
      qtd: 100,
      custo: 18,
      forn: 'Tech Import Brasil',
    ...
    {
      sku: 'CS-PERF-001',
      produto: 'Perfume Essence Black 30ml',
      data: '2026-04-20',
      qtd: 80,
      custo: 14,
      forn: 'Distribuidora Alfa Beleza',
    ...
    {
      sku: 'CS-CASA-003',
      produto: 'Kit Panos Microfibra 5un',
      data: '2026-04-25',
      qtd: 100,
      custo: 10,
      forn: 'Casa & Decoração LTDA',
      pag: 'Pix',
    },
    {
      sku: 'CS-PERF-023',
      produto: 'Desodorante Roll-on 24h 50ml',
      data: '2026-03-10',
      qtd: 150,
      custo: 6,
      forn: 'Distribuidora Alfa Beleza',
    ...
    {
      sku: 'CS-ELET-013',
      produto: 'Película Vidro Temperado 9H',
      data: '2026-03-15',
      qtd: 300,
      custo: 3,
      forn: 'Tech Import Brasil',
    ...
    {
      sku: 'CP-PERF-001',
      produto: 'Perfume Importado Noir 100ml',
      data: '2026-02-01',
      qtd: 20,
      custo: 85,
      forn: 'Distribuidora Alfa Beleza',
    ...
    {
      sku: 'CS-SAUD-001',
      produto: 'Termômetro Digital Infravermelho',
      data: '2026-02-20',
      qtd: 30,
      custo: 28,
      forn: 'Prime Saúde Distribuidora',
      pag: 'Pix',
    },
    {
      sku: 'CS-ACES-001',
      produto: 'Óculos Proteção UV400',
      data: '2026-01-15',
      qtd: 50,
      custo: 18,
      forn: 'Fashion Acessórios SP',
      pag: '30 dias',
    },
  ];
  const compraRows = [];
  ...
  for (const c of COMPRAS) {
    ...
    compraRows.push({
      ...
      produto: c.produto,
      data: c.data,
      quantidade_entrada: c.qtd,
      custo_unitario: c.custo,
  ...
  const DESP_OP = [
    {
      data: '2026-06-01',
      cat: 'Embalagem',
      desc: 'Caixas kraft P, M, G — Fatura Junho',
      val: 420,
      loja: LOJA_A,
    },
    {
      data: '2026-06-01',
      cat: 'Embalagem',
      desc: 'Envelopes plásticos 100un',
      val: 85,
      loja: LOJA_A,
    },
    {
      data: '2026-06-03',
      cat: 'Marketing',
      desc: 'Shopee Ads — Campanha Queima Inverno',
      val: 600,
      loja: LOJA_A,
    },
    {
    ...
    },
    {
      data: '2026-06-04',
      cat: 'Outro',
      desc: 'Assinatura Shopee Gestão Pro',
      val: 89,
      loja: 'Ambas',
    },
    {
    ...
    },
    {
      data: '2026-05-05',
      cat: 'Marketing',
      desc: 'Shopee Ads — Dia das Mães (boost)',
      val: 2200,
      loja: LOJA_A,
    },
    {
      data: '2026-04-10',
      cat: 'Marketing',
      desc: 'Shopee Ads — Dia das Mães (lançamento)',
      val: 1800,
      loja: LOJA_A,
    },
    {
      data: '2026-04-05',
      cat: 'Embalagem',
      desc: 'Caixas e embalagens — Abril',
      val: 420,
      loja: LOJA_A,
    },
    { data: '2026-03-10', cat: 'Embalagem', desc: 'Embalagens Março', val: 380, loja: LOJA_A },
    {
      data: '2026-03-01',
      cat: 'Combustível',
      desc: 'Gasolina — entregas locais Março',
      val: 120,
      loja: 'Ambas',
    },
    ...
    { data: '2026-01-10', cat: 'Embalagem', desc: 'Embalagens Janeiro', val: 340, loja: LOJA_A },
    {
      data: '2025-12-05',
      cat: 'Embalagem',
      desc: 'Embalagens Especiais Natal',
      val: 650,
      loja: LOJA_A,
    },
    {
      data: '2025-11-02',
      cat: 'Embalagem',
      desc: 'Embalagens Black Friday (estoque extra)',
      val: 580,
      loja: LOJA_A,
    },
    {
      data: '2025-11-01',
      cat: 'Marketing',
      desc: 'Shopee Ads — Black November',
      val: 1200,
      loja: LOJA_A,
    },
  ];
  ...
  const histRows = HISTORICO_DEF.map((h) => {
    ...
    return {
      mes_ano: h.mesAno,
      user_id: uid,
      faturamento_bruto: h.fat,
      pedidos_qtd: h.nPed,
      ticket_medio: parseFloat((h.fat / h.nPed).toFixed(2)),
      unidades_vendidas: h.uni,
      cmv: h.cmv,
      taxas_shopee: h.taxShopee,
      das_imposto: h.das,
      marketing_ads: h.ads,
      despesas_operacionais: h.desp,
  ...
  const fornRows = FORNECEDORES.map((f) => ({
    ...
    termos_pagamento: f.termosPagamento,
  ...
  for (const mes of ['2026-05', '2026-06']) {
    for (const p of topSkus) {
      metaRows.push({
        ...
        sku: p.sku,
        mes_ano: mes,
        meta_unidades: p.metaUni,
        meta_receita: p.metaRec,
      });
    }
  }
  {
